...
conn.on('ready', () => {
    conn.exec('cd domains/sukounalbunyan.ae/public_html/backend && tail -n 1500 storage/logs/noble-2026-05-05.log | grep -i -A 10 "production.ERROR" | tail -n 25', (err, stream) => {
        if (err) throw err;

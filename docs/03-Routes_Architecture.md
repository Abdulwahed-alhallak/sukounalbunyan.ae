...
- `/MissionCommand/gamification`: نظام التلعيب والمكافآت.
- `/MissionCommand/dionflow`: نظام أتمتة الإجراءات (No-code Engine).
- `/MissionCommand/unified-inbox`: صندوق المحادثات الموحد للرد على العملاء.
...
## 8. مسارات لوحات التحكم الديناميكية (Dynamic Dashboards)

يعتمد النظام منطق توجيه متطور للوحات التحكم الخاصة بالموديولات (مثل Taskly و Accounting) حيث يتم تحديد نوع لوحة التحكم (Company, Staff, Client, Vendor) ديناميكياً بناءً على هوية المستخدم المسجل:

- **Taskly Dashboard:** `/taskly/dashboard` - تم إصلاح منطق العرض في `DashboardController.php` لضمان توجيه المستخدمين لصفحة `CompanyDashboard` أو `StaffDashboard` بدلاً من إظهار أخطاء 403 غير المبررة.
- **Accounting Dashboard:** يتم توحيد Breadcrumbs في كافة إصدارات لوحة التحكم لضمان تجربة مستخدم متناسقة.
...
## 9. الربط مع المحرك الأمامي (Inertia Routing Bridge)

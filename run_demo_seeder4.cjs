...
conn.on('ready', () => {
    conn.exec('cd domains/sukounalbunyan.ae/public_html/backend && git pull origin master && /opt/alt/php82/usr/bin/php artisan db:seed --class=DemoRentalDataSeeder --force', (err, stream) => {
        if (err) throw err;

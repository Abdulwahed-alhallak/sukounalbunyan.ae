    ...
    <div className={cn("relative w-full max-w-full", className)}>
      ...
      {(showZoom || showDownload || showCloseButton) && (
        <div className="absolute top-3 right-3 z-20 flex gap-1">
          {showZoom && (
            <Button variant="outline" size="icon" className="h-8 w-8 bg-card/90 hover:bg-card shadow-sm" onClick={() => setIsZoomed(!isZoomed)}>
              <ZoomIn className="h-3 w-3" />
            </Button>
          )}
          {showDownload && (
            <Button variant="outline" size="icon" className="h-8 w-8 bg-card/90 hover:bg-card shadow-sm" onClick={() => {
          ...
          {showCloseButton && onClose && (
            <Button variant="outline" size="icon" className="h-8 w-8 bg-card/90 hover:bg-card shadow-sm" onClick={onClose}>
              <X className="h-3 w-3" />
            </Button>
          )}
        </div>
      ...
      <div className={cn("relative overflow-hidden rounded-lg bg-muted border", aspectRatioClass)}>
        ...
        {showControls && processedImages.length > 1 && (
          <>
            <Button
              variant="outline"
              size="icon"
              className="absolute left-3 top-1/2 -translate-y-1/2 h-8 w-8 bg-card/90 hover:bg-card shadow-sm"
            ...
            <Button
              variant="outline"
              size="icon"
              className="absolute right-3 top-1/2 -translate-y-1/2 h-8 w-8 bg-card/90 hover:bg-card shadow-sm"
            ...
            {processedImages.map((_, index) => (
              <button
                ...
                className={cn(
                  ...
                  index === currentIndex ? "bg-card scale-125" : "bg-card/60 hover:bg-card/80"
                )}
        ...
        {/* Image Counter */}
        {processedImages.length > 1 && (
          <div className="absolute top-3 left-3 bg-foreground/60 text-background rounded px-2 py-1 text-xs font-medium">
          ...
          {processedImages.map((image, index) => (
            <button
              ...
              className={cn(
                ...
                index === currentIndex ? "border-foreground ring-2 ring-foreground/20" : "border-border hover:border-foreground/50"
              )}
              onClick={() => setCurrentIndex(index)}
            >

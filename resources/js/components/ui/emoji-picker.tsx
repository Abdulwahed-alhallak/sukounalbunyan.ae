...
export function EmojiPicker({ onEmojiSelect, className = '' }: EmojiPickerProps) {
    ...
    const filtered = useMemo(() => {
        ...
        Object.entries(categorized).forEach(([cat, items]) => {
            const matched = items.filter((e) => e.name.toLowerCase().includes(lower) || e.emoji.includes(lower));
    ...
    return (
        <div className={`z-10 w-80 rounded-lg border bg-background p-4 shadow-lg ${className}`}>
            {/* Search input */}
            <input
                type="text"
                ...
                className="mb-3 w-full rounded-md border px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-border"
            />

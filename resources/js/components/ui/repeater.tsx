...
export interface RepeaterProps {
    ...
    renderCustomField?: (
        item: RepeaterItem,
        index: number,
        updateItem: (fieldName: string, value: any) => void
    ) => React.ReactNode;
                    ...
                    ...fields.reduce(
                        (acc, field) => {
                            if (field.type === 'checkbox' || field.type === 'switch') {
                                ...
                                acc[field.name] =
                                    firstCondition.type === 'checkbox' || firstCondition.type === 'switch' ? false : '';
                            } else {
                                acc[field.name] = '';
                            }
                            return acc;
        ...
        const renderField = (field: RepeaterField, item: RepeaterItem) => {
            ...
            switch (field.type) {
                        ...
                        >
                            ...
                            <SelectContent searchable={(field.options?.length || 0) > 10}>
                                {field.options?.map((option, optionIndex) => (
                                    <SelectItem
                                        key={`${item.id}-${field.name}-${option.value}-${optionIndex}`}
                                        value={option.value}
                                    >
                                        {option.label}
                                    </SelectItem>
                ...
                case 'checkbox':
                    return (
                        <div
                            key={`checkbox-wrapper-${item.id}-${field.name}`}
                    ...
                    return (
                        ...
                        >
                            {field.options?.map((option, index) => {
                                ...
                                return (
                                    <div
                                        key={`${item.id}-${field.name}-${option.value}-${index}`}
                        ...
                        <Repeater
                            ...
                            fields={
                                field.options && field.options.length > 0
                                    ? field.options.map((opt) => ({ type: 'text', name: opt.value, label: opt.label }))
                    ...
                    <div className="max-h-96 space-y-4 overflow-y-auto pe-2">
                        {(Array.isArray(value) ? value : []).map((item, index) => (
                            <div
                                ...
                                className="bg-muted/50/50 rounded-lg border p-4"
                            >
                                ...
                                <div className={getLayoutClassName()}>
                                    {fields
                                        ...
                                        .map((field, fieldIndex) => {
                                            ...
                                            const fieldStyle = field.layout?.width
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    {(errors[item.id]?.[field.name] ||
                                                        (showValidation &&
                                                            validationErrors[item.id]?.[field.name])) && (
                                ...
                                {renderCustomField &&
                                    renderCustomField(item, index, (fieldName: string, fieldValue: any) =>
                                        updateItem(item.id, fieldName, fieldValue)
                                    )}
                            </div>
                        ))}
                    </div>

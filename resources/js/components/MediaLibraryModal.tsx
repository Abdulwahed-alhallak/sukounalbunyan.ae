...
export default function MediaLibraryModal({ 
  ...
  const getFileIcon = (mimeType: string) => {
    ...
    if (mimeType.includes('pdf')) return <div className="h-8 w-8 bg-foreground rounded text-background text-sm flex items-center justify-center font-bold">PDF</div>;
    if (mimeType.includes('word') || mimeType.includes('document')) return <div className="h-8 w-8 bg-foreground rounded text-background text-sm flex items-center justify-center font-bold">DOC</div>;
    if (mimeType.includes('csv') || mimeType.includes('spreadsheet')) return <div className="h-8 w-8 bg-foreground rounded text-background text-sm flex items-center justify-center font-bold">CSV</div>;
    if (mimeType.startsWith('video/')) return <div className="h-8 w-8 bg-foreground rounded text-background text-sm flex items-center justify-center font-bold">VID</div>;
    if (mimeType.startsWith('audio/')) return <div className="h-8 w-8 bg-foreground rounded text-background text-sm flex items-center justify-center font-bold">AUD</div>;
    return <div className="h-8 w-8 bg-muted/500 rounded text-background text-sm flex items-center justify-center font-bold">FILE</div>;
  };
                ...
                <div className="text-center max-w-sm">
                  <div
                    className={`mx-auto w-24 h-24 border-2 border-dashed rounded-xl flex items-center justify-center mb-6 transition-colors ${
                      dragActive ? 'border-foreground bg-foreground/5' : 'border-muted-foreground/25'
                    }`}
                      ...
                      <div
                        ...
                        className={`relative group cursor-pointer rounded-lg overflow-hidden transition-all hover:scale-105 ${
                          selectedItems.includes(item.url) 
                            ...
                            : 'hover:shadow-md border border-border hover:border-foreground/50'
                        }`}
                      ...
                      >
                        <div className="relative aspect-square bg-muted">
                          ...
                          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <p className="text-xs text-background truncate" title={item.name}>
                              {item.name}
                            </p>
                          </div>
                        </div>

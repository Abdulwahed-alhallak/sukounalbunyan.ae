...
export default function SocialLinks({ socialLinks, icon, className = '', style, textSecondary = '#64748b', variant = 'default', size = 'md', userSlug }: SocialLinksProps) {
    const renderIcon = (iconName: string) => {
        ...
        if (iconName.startsWith('svg:')) {
            ...
            return <div className={`${iconClasses} bg-muted rounded flex items-center justify-center text-xs`}>?</div>;
        }
        ...
        return <div className={`${iconClasses} bg-muted rounded`} />;
    };
    ...
    if (icon && typeof icon === 'string') {
        ...
        return <div className={`w-5 h-5 bg-muted rounded ${className}`} style={style} />;
    }

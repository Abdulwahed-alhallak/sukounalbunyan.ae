                        ...
                        {auth.user?.permissions?.includes('edit-warehouses') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', warehouse)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                        ...
                                        <Card key={warehouse.id} className="border border-border">
                                            <div className="p-4">
                                                    ...
                                                    <div className="flex-1">
                                                        <h3 className="font-semibold text-base text-foreground">{warehouse.name}</h3>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="space-y-3 mb-3">
                                                    ...
                                                    <div>
                                                        <p className="text-xs font-medium text-muted-foreground mb-2">{t('Address')}</p>
                                                        <p className="text-xs text-foreground truncate" title={warehouse.address}>{warehouse.address}</p>
                                                    </div>

                                                    <div className="grid grid-cols-2 gap-2">
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('City')}</p>
                                                            <p className="text-xs text-foreground truncate">{warehouse.city}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Zip')}</p>
                                                            <p className="text-xs text-foreground">{warehouse.zip_code}</p>
                                                        </div>
                                                    </div>

                                                    {warehouse.phone && (
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Phone')}</p>
                                                            <p className="text-xs text-foreground">{warehouse.phone}</p>
                                                        </div>
                                                    )}
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between pt-3 border-t">
                                                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${
                                                        warehouse.is_active ? 'bg-muted text-foreground' : 'bg-muted text-destructive'
                                                    }`}>
                                                        ...
                                                        <TooltipProvider>
                                                            {auth.user?.permissions?.includes('edit-warehouses') && (
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => openModal('edit', warehouse)} className="h-8 w-8 p-0 text-foreground">
                                                                            <Edit className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>

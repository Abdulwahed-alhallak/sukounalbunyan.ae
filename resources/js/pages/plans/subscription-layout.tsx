        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-3 space-y-6">
                ...
                <div className="bg-card dark:bg-card rounded-lg border border-border dark:border-border p-6">
                    <h3 className="text-lg font-semibold text-foreground dark:text-foreground mb-4">{t('Plan Details')}</h3>
                    <div className="space-y-3">                         
                        ...
                        <div className="flex justify-between">
                            <span className="text-muted-foreground dark:text-muted-foreground">{t('Plan Expire Date')}</span>
                            <span className="font-medium text-foreground dark:text-foreground">
                ...
                <div className="bg-card dark:bg-card rounded-lg border border-border dark:border-border p-6">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-semibold text-foreground dark:text-foreground">{t('Available Features')}</h3>
                        <SearchInput
                            value={moduleSearch}
            ...
            <div className="lg:col-span-2 bg-card dark:bg-card rounded-lg border border-border dark:border-border p-6">
                <h3 className="text-lg font-semibold text-foreground dark:text-foreground mb-6">
                    {t('Subscribe to Plan')}
                </h3>
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        <div className="flex items-center justify-between p-3 border border-border dark:border-border rounded-lg">
                            ...
                            <span className="text-sm text-muted-foreground dark:text-muted-foreground">{plan.number_of_users === -1 ? t('Unlimited Users') : plan.number_of_users}</span>
                        </div>
                        <div className="flex items-center justify-between p-3 border border-border dark:border-border rounded-lg">
                            <div className="flex items-center space-x-2">
                                <svg className="w-4 h-4 text-foreground" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <span className="text-sm text-muted-foreground dark:text-muted-foreground">{formatStorage(plan.storage_limit)}</span>
                        </div>
                        {plan.trial && (
                            <div className="flex items-center justify-between p-3 border border-border dark:border-border bg-muted dark:bg-muted rounded-lg">
                                <div className="flex items-center space-x-2">
                                    <svg className="w-4 h-4 text-foreground" fill="currentColor" viewBox="0 0 20 20">
                                        <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                                    </svg>
                                    <span className="text-sm font-medium text-foreground dark:text-muted-foreground">{t('Free Trial')}</span>
                                </div>
                                <span className="text-sm text-foreground dark:text-foreground">{plan.trial_days} {t('days')}</span>
                            </div>
                        )}
                    </div>
                    ...
                    {(bankTransferEnabled || paymentButtons.length > 0) && (
                        <div className="space-y-4">
                            ...
                            <RadioGroup value={selectedPaymentMethod || ''} onValueChange={(value) => {
                                ...
                                {bankTransferEnabled && (
                                    <>
                                        <div className="flex items-center space-x-3 p-3 border border-border dark:border-border rounded-lg w-full">
                                            <RadioGroupItem value="bank_transfer" id="bank_transfer" />
                                            <Label htmlFor="bank_transfer" className="cursor-pointer">
                                                <div className="font-medium text-foreground dark:text-foreground">{t('Bank Transfer')}</div>
                                                <div className="text-sm text-muted-foreground dark:text-muted-foreground">{t('Pay via bank transfer')}</div>
                                            </Label>
                                        </div>
                                        <div className="flex items-center space-x-3 p-3 border border-border dark:border-border rounded-lg w-full">
                                            <RadioGroupItem value="cash_on_hand" id="cash_on_hand" />
                                            <Label htmlFor="cash_on_hand" className="cursor-pointer">
                                                <div className="font-medium text-foreground dark:text-foreground">{t('Cash on Hand (الدفع داخل شركة DION)')}</div>
                                                <div className="text-sm text-muted-foreground dark:text-muted-foreground">{t('ادفع نقداً أو من خلال زيارة مقر الشركة. يمكن لموظف دايون تفعيل الاشتراك مباشرة.')}</div>
                                            </Label>
                                        </div>
                                    </>
                                )}                                
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-4">
                                        {bankTransferInstructions && (
                                            <div className="p-3 bg-muted dark:bg-muted border border-border dark:border-border rounded-lg">
                                                <div className="text-sm text-foreground dark:text-muted-foreground" dangerouslySetInnerHTML={{ __html: bankTransferInstructions.replace(/\n/g, '<br/>') }} />
                                            </div>
                                        )}

...
export default function Index() {
    ...
    const tableColumns = [
        ...
        },
        {
            ...
            sortable: true,
            render: (value: string) => (
                <span className="font-mono bg-muted px-2 py-1 rounded-full text-sm">
                    {value}
                </span>
            )
        },
                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        ...
                        {auth.user?.permissions?.includes('edit-coupons') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', coupon)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                ...
                {showFilters && (
                    <CardContent className="p-6 bg-muted/30 border-b">
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Code')}</label>
                                <Input
                                            ...
                                            <div className="p-4">
                                                <div className="flex items-center justify-between mb-3">
                                                    ...
                                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                                        coupon.status ? 'bg-muted text-foreground' : 'bg-muted text-destructive'
                                                    }`}>
                                                ...
                                                <div className="space-y-2 mb-3">
                                                    <div className="text-xs">
                                                        <p className="text-muted-foreground mb-1">{t('Code')}</p>
                                                        <p className="font-mono bg-muted px-2 py-1 rounded text-xs font-medium text-foreground">{coupon.code}</p>
                                                    </div>
                                                    
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <div className="text-xs">
                                                            <p className="text-muted-foreground mb-1">{t('Discount')}</p>
                                                            <p className="font-medium text-foreground">
                                                                {coupon.type === 'percentage' ? `${coupon.discount}%` : `${currencySymbol}${coupon.discount}`}
                                                            </p>
                                                        </div>
                                                        <div className="text-xs">
                                                            <p className="text-muted-foreground mb-1">{t('Type')}</p>
                                                            <p className="font-medium text-foreground capitalize">{coupon.type}</p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    {(coupon.limit || coupon.expiry_date) && (
                                                        <div className="grid grid-cols-2 gap-2">
                                                            {coupon.limit && (
                                                                <div className="text-xs">
                                                                    <p className="text-muted-foreground mb-1">{t('Limit')}</p>
                                                                    <p className="font-medium text-foreground">{coupon.limit}</p>
                                                                </div>
                                                            )}
                                                            {coupon.expiry_date && (
                                                                <div className="text-xs">
                                                                    <p className="text-muted-foreground mb-1">{t('Expiry')}</p>
                                                                    <p className="font-medium text-foreground">{formatDate(coupon.expiry_date, pageProps)}</p>
                                                                </div>
                                                            )}
                                                        </div>
                                                    )}
                                                ...
                                                <div className="flex justify-end gap-1 pt-2 border-t">
                                                    <TooltipProvider>
                                                        ...
                                                        {auth.user?.permissions?.includes('edit-coupons') && (
                                                            <Tooltip delayDuration={300}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', coupon)} className="h-7 w-7 p-0 text-foreground hover:text-foreground">

    ...
    const tableColumns = [
        {
            ...
            render: (value: string, invoice: SalesInvoice) =>
                auth.user?.permissions?.includes('view-sales-invoices') ? (
                    <span className="text-foreground hover:text-foreground cursor-pointer" onClick={() => router.get(route('sales-invoices.show', invoice.id))}>{value}</span>
                ) : (
                    value
                )
        },
                                        ...
                                        <Card key={invoice.id} className="border border-border flex flex-col">
                                            <div className="p-4 flex-1">
                                                <div className="flex items-center justify-between mb-3">
                                                    {auth.user?.permissions?.includes('view-sales-invoices') ? (
                                                        <h3 className="font-semibold text-base text-foreground hover:text-foreground cursor-pointer" onClick={() => router.get(route('sales-invoices.show', invoice.id))}>{invoice.invoice_number}</h3>
                                                    ) : (
                                                ...
                                                <div className="space-y-3 mb-4">
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-foreground truncate font-medium">{invoice.customer?.name}</p>
                                                    </div>
                                             ...
                                             <div className="flex gap-1">
                                                    <TooltipProvider>
                                                        ...
                                                        {auth.user?.permissions?.includes('print-sales-invoices') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => window.open(route('sales-invoices.print', invoice.id) + '?download=pdf', '_blank')} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <Download className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                        ...
                                                        {auth.user?.permissions?.includes('view-sales-invoices') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('sales-invoices.show', invoice.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <Eye className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                        ...
                                                        {invoice.status === 'draft' && (
                                                            <>
                                                                {auth.user?.permissions?.includes('post-sales-invoices') && (
                                                                    <Tooltip delayDuration={0}>
                                                                        <TooltipTrigger asChild>
                                                                            <Button variant="ghost" size="sm" onClick={() => router.post(route('sales-invoices.post', invoice.id))} className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground">
                                                                                <FileText className="h-4 w-4" />
                                                                            </Button>
                                                                        </TooltipTrigger>
                                                                ...
                                                                {auth.user?.permissions?.includes('edit-sales-invoices') && (
                                                                    <Tooltip delayDuration={0}>
                                                                        <TooltipTrigger asChild>
                                                                            <Button variant="ghost" size="sm" onClick={() => router.visit(route('sales-invoices.edit', invoice.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                                <EditIcon className="h-4 w-4" />
                                                                            </Button>
                                                                        </TooltipTrigger>

    ...
    const tableColumns = [
        {
            ...
            render: (value: string, invoice: PurchaseInvoice) =>
                auth.user?.permissions?.includes('view-purchase-invoices') ? (
                    <span className="text-foreground hover:text-foreground cursor-pointer" onClick={() => router.get(route('purchase-invoices.show', invoice.id))}>{value}</span>
                ) : (
                    value
                )
        },
                            ...
                            {invoices.data.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                    {invoices.data.map((invoice) => (
                                        <Card key={invoice.id} className="border border-border flex flex-col">
                                            <div className="p-4 flex-1">
                                                <div className="flex items-center justify-between mb-3">
                                                    {auth.user?.permissions?.includes('view-purchase-invoices') ? (
                                                        <h3 className="font-semibold text-base text-foreground hover:text-foreground cursor-pointer" onClick={() => router.get(route('purchase-invoices.show', invoice.id))}>{invoice.invoice_number}</h3>
                                                    ) : (
                                                        <h3 className="font-semibold text-base text-foreground">{invoice.invoice_number}</h3>
                                                    )}
                                                ...
                                                <div className="space-y-3 mb-4">
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-foreground truncate font-medium">{invoice.vendor?.name}</p>
                                                    </div>
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Invoice Date')}</p>
                                                            <p className="text-xs text-foreground">{formatDate(invoice.invoice_date)}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Due Date')}</p>
                                                            <p className={`text-xs ${invoice.display_status === 'overdue' ? 'text-destructive font-medium' : 'text-foreground'}`}>
                                                    ...
                                                    <div className="bg-muted/50 rounded-lg p-3">
                                                        ...
                                                        <div className="border-t mt-2 pt-2">
                                                            ...
                                                            <div className="flex justify-between items-center mt-1">
                                                                ...
                                                                <span className="text-sm font-semibold text-foreground">{formatCurrency(invoice.balance_amount)}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                             ...
                                             <div className="flex gap-1">
                                                    <TooltipProvider>
                                                        ...
                                                        {auth.user?.permissions?.includes('print-purchase-invoices') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => window.open(route('purchase-invoices.print', invoice.id) + '?download=pdf', '_blank')} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <Download className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                        ...
                                                        {auth.user?.permissions?.includes('view-purchase-invoices') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('purchase-invoices.show', invoice.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <Eye className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                        ...
                                                        {invoice.status === 'draft' && (
                                                            <>
                                                                {auth.user?.permissions?.includes('post-purchase-invoices') && (
                                                                    <Tooltip delayDuration={0}>
                                                                        <TooltipTrigger asChild>
                                                                            <Button variant="ghost" size="sm" onClick={() => router.post(route('purchase-invoices.post', invoice.id))} className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground">
                                                                                <FileText className="h-4 w-4" />
                                                                            </Button>
                                                                        </TooltipTrigger>
                                                                ...
                                                                {auth.user?.permissions?.includes('edit-purchase-invoices') && (
                                                                    <Tooltip delayDuration={0}>
                                                                        <TooltipTrigger asChild>
                                                                            <Button variant="ghost" size="sm" onClick={() => router.visit(route('purchase-invoices.edit', invoice.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                                <EditIcon className="h-4 w-4" />
                                                                            </Button>
                                                                        </TooltipTrigger>

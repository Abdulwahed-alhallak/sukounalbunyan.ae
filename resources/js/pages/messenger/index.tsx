...
export default function MessengerPage() {
    ...
    const renderAttachment = (message: Message & { _tempFile?: File | null }) => {
        ...
        if (message._tempFile) {
            ...
            return (
                <div className="mt-2 flex items-center gap-2 p-2 bg-card/10 rounded-lg">
                            ...
                            <div>
                                {filteredUsers.map((user: ChatUser) => (
                                    <div
                                        ...
                                        className={`group flex items-center gap-3 p-3 cursor-pointer transition-all duration-150 border-b border-border ${
                                            selectedUser?.id === user.id 
                                                ? 'bg-muted border-r-4 border-foreground' 
                                                : 'hover:bg-muted/50'
                                        } ${pinnedUsers.includes(user.id) ? 'bg-muted border-l-2 border-border' : ''}`}
                                    >
                                        <div className="relative">
                                            ...
                                            {pinnedUsers.includes(user.id) && (
                                                <div className="absolute -top-1 -left-1 bg-foreground text-background rounded-full w-5 h-5 flex items-center justify-center text-xs">
                                                    📌
                                                </div>
                                            )}
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center justify-between mb-1">
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        {user.unread_count > 0 && (
                                                            <div className="bg-foreground text-background rounded-full min-w-[18px] h-[18px] flex items-center justify-center text-xs font-medium">
                                                ...
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity ml-2">
                                                        ...
                                                        >
                                                            <span className={`text-xs ${pinnedUsers.includes(user.id) ? 'text-foreground' : 'text-muted-foreground'}`}>
                                ...
                                <div className={`text-center py-12 text-muted-foreground ${filteredUsers.length === 0 ? '' : 'hidden'}`}>
                    ...
                    <CardHeader className={`pb-3 ${selectedUser ? 'border-b' : 'hidden'}`}>
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">{selectedUser?.is_online ? t('Online') : t('Offline')}</p>
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent className="flex-1 p-0 flex flex-col">
                            ...
                            >
                                {isLoadingMessages && currentPage > 1 && (
                                    <div className="flex justify-center py-3 mb-2">
                                        <div className="text-xs text-muted-foreground bg-muted px-3 py-1 rounded-full animate-pulse">
                                            {t('Loading messages...')}
                                        </div>
                                    </div>
                                )}
                                            ...
                                            <div key={message.id} className={`group flex mb-2 items-end gap-2 ${isOwnMessage ? 'justify-end' : 'justify-start'}`}>
                                                {!isOwnMessage && (auth.user?.permissions?.includes('delete-messages')) && (
                                                    <div className="relative opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <Button
                                                            ...
                                                            className="h-6 w-6 p-0 rounded-full text-muted-foreground hover:text-muted-foreground hover:bg-muted"
                                                        >
                                                            <MoreVertical className="h-4 w-4" />
                                                        </Button>
                                                        {openDropdown === message.id && (
                                                            <div className="absolute left-0 top-7 bg-card shadow-xl rounded-lg py-2 min-w-[140px] z-20 border">
                                                                {auth.user?.permissions?.includes('delete-messages') && (
                                                                    <button
                                                                        ...
                                                                        className="w-full px-4 py-2 text-left text-sm text-destructive hover:bg-muted flex items-center gap-3 transition-colors"
                                                                    >
                                                                        <Trash2 className="h-4 w-4 text-destructive" /> {t('Delete')}
                                                                    </button>
                                                                )}
                                                            </div>
                                                        )}
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-center justify-end gap-1 mt-1">
                                                            <span className={`text-xs ${
                                                                isOwnMessage ? 'text-foreground-100' : 'text-muted-foreground'
                                                            }`}>
                                                                {new Date(message.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                            </span>
                                                            {isOwnMessage && (
                                                                <span className={`text-xs ${
                                                                    message.is_read ? 'text-muted-foreground' : 'text-foreground-200'
                                                                }`}>
                                                ...
                                                {isOwnMessage && (auth.user?.permissions?.includes('edit-messages') || auth.user?.permissions?.includes('delete-messages')) && (
                                                    <div className="relative opacity-0 group-hover:opacity-100 transition-opacity">
                                                        ...
                                                        {openDropdown === message.id && (
                                                            <div className="absolute right-0 top-7 bg-card shadow-xl rounded-lg py-2 min-w-[140px] z-20 border">
                                                                {isOwnMessage && auth.user?.permissions?.includes('edit-messages') && (
                                                                    <button
                                                                        ...
                                                                        className="w-full px-4 py-2 text-left text-sm text-foreground hover:bg-muted/50 flex items-center gap-3 transition-colors"
                                                                    >
                                                                        <Edit className="h-4 w-4 text-foreground" /> {t('Edit')}
                                                                    </button>
                                                                )}
                                                                {auth.user?.permissions?.includes('delete-messages') && (
                                                                    <button
                                    ...
                                    <form onSubmit={handleSendMessage} className="relative">
                                        <div className="flex items-center gap-3">
                                            <div className="relative flex-1">
                                                <div className="relative">
                                                    {selectedFile && (
                                                        <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1 bg-muted rounded px-2 py-1 z-10">
                        ...
                        <div className={`flex-1 flex items-center justify-center ${selectedUser ? 'hidden' : ''}`}>
                            <div className="text-center text-muted-foreground">
                                <div className="p-6 bg-foreground/5 rounded-full w-24 h-24 mx-auto mb-6 flex items-center justify-center">
                                    <MessageCircle className="h-12 w-12 text-foreground/60" />
                                </div>
                                <h3 className="text-xl font-semibold text-foreground mb-2">{t('Select a conversation')}</h3>
                                <p className="text-muted-foreground">{t('Choose a user from the list to start messaging')}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>

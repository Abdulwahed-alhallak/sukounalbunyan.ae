...
export default function Index() {
    ...
    const getProposalStatusColor = (status: string) => {
        switch (status?.toLowerCase()) {
            ...
            case 'sent': return 'bg-muted text-foreground';
            case 'accepted': return 'bg-muted text-foreground';
    ...
    const tableColumns = [
        {
            ...
            render: (value: string, proposal: SalesProposal) =>
                auth.user?.permissions?.includes('view-sales-proposals') ? (
                    <span className="text-foreground hover:text-foreground cursor-pointer" onClick={() => router.get(route('sales-proposals.show', proposal.id))}>{value}</span>
                ) : (
                    `${value}`
                )
        },
                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('print-sales-proposals') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => window.open(route('sales-proposals.print', item.id) + '?download=pdf', '_blank')} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('sent-sales-proposals') && item.status === 'draft' && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.post(route('sales-proposals.sent', item.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('accept-sales-proposals') && item.status === 'sent' && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.post(route('sales-proposals.accept', item.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('reject-sales-proposals') && item.status === 'sent' && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.post(route('sales-proposals.reject', item.id))} className="h-8 w-8 p-0 text-destructive hover:text-destructive">
                                        <X className="h-4 w-4" />
                                    </Button>
                                </TooltipTrigger>
                        ...
                        {item.converted_to_invoice ? (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('sales-invoices.show', item.invoice_id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                            ...
                            auth.user?.permissions?.includes('convert-sales-proposals') && item.status === 'accepted' && (
                                <Tooltip delayDuration={0}>
                                    <TooltipTrigger asChild>
                                        <Button variant="ghost" size="sm" onClick={() => openConvertDialog(item.id)} className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('view-sales-proposals') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('sales-proposals.show', item.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {item.status === 'draft' && auth.user?.permissions?.includes('edit-sales-proposals') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.visit(route('sales-proposals.edit', item.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                            ...
                            {proposals?.data?.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                    {proposals.data.map((proposal) => (
                                        <Card key={proposal.id} className="border border-border flex flex-col">
                                            <div className="p-4 flex-1">
                                                <div className="flex items-center justify-between mb-3">
                                                    {auth.user?.permissions?.includes('view-sales-proposals') ? (
                                                        <h3 className="font-semibold text-base text-foreground hover:text-foreground cursor-pointer" onClick={() => router.get(route('sales-proposals.show', proposal.id))}>{proposal.proposal_number}</h3>
                                                    ) : (
                                                        <h3 className="font-semibold text-base text-foreground">{proposal.proposal_number}</h3>
                                                    )}
                                                ...
                                                <div className="space-y-3 mb-4">
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-foreground truncate font-medium">{proposal.customer?.name}</p>
                                                    </div>
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Proposal Date')}</p>
                                                            <p className="text-xs text-foreground">{formatDate(proposal.proposal_date)}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Due Date')}</p>
                                                            <p className={`text-xs ${proposal.display_status === 'overdue' ? 'text-destructive font-medium' : 'text-foreground'}`}>
                                                    ...
                                                    <div className="bg-muted/50 rounded-lg p-3">
                                                        ...
                                                        <div className="border-t mt-2 pt-2">
                                                            <div className="flex justify-between items-center">
                                                                ...
                                                                <span className="text-lg font-bold text-foreground">{formatCurrency(proposal.total_amount)}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                             ...
                                             <div className="flex gap-1">
                                                    <TooltipProvider>
                                                        {auth.user?.permissions?.includes('print-sales-proposals') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => window.open(route('sales-proposals.print', proposal.id) + '?download=pdf', '_blank')} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <Download className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                                <TooltipContent><p>{t('Download PDF')}</p></TooltipContent>
                                                        ...
                                                        {auth.user?.permissions?.includes('view-sales-proposals') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('sales-proposals.show', proposal.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <Eye className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                                <TooltipContent><p>{t('View')}</p></TooltipContent>
                                                ...
                                                <div className="flex gap-1">
                                                    <TooltipProvider>
                                                        {auth.user?.permissions?.includes('sent-sales-proposals') && proposal.status === 'draft' && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.post(route('sales-proposals.sent', proposal.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <Send className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                                <TooltipContent><p>{t('Send Proposal')}</p></TooltipContent>
                                                        ...
                                                        {auth.user?.permissions?.includes('accept-sales-proposals') && proposal.status === 'sent' && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.post(route('sales-proposals.accept', proposal.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <Check className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                                <TooltipContent><p>{t('Accept Proposal')}</p></TooltipContent>
                                                        ...
                                                        {auth.user?.permissions?.includes('reject-sales-proposals') && proposal.status === 'sent' && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.post(route('sales-proposals.reject', proposal.id))} className="h-8 w-8 p-0 text-destructive hover:text-destructive">
                                                                        <X className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                                <TooltipContent><p>{t('Reject Proposal')}</p></TooltipContent>
                                                        ...
                                                        {proposal.converted_to_invoice ? (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('sales-invoices.show', proposal.invoice_id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <Receipt className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                                <TooltipContent><p>{t('View Invoice')}</p></TooltipContent>
                                                            ...
                                                            auth.user?.permissions?.includes('convert-sales-proposals') && proposal.status === 'accepted' && (
                                                                <Tooltip delayDuration={0}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => openConvertDialog(proposal.id)} className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground">
                                                                            <RefreshCw className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent><p>{t('Convert to Invoice')}</p></TooltipContent>
                                                        ...
                                                        {proposal.status === 'draft' && auth.user?.permissions?.includes('edit-sales-proposals') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.visit(route('sales-proposals.edit', proposal.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <EditIcon className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                                <TooltipContent><p>{t('Edit')}</p></TooltipContent>

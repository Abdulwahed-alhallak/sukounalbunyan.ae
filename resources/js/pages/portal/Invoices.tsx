...
export default function PortalInvoices({ invoices, type }: Props) {
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[
                { label: t('Portal'), url: route('portal.dashboard') },
                { label: isClient ? t('Invoices') : t('Bills') },
            ]}
        >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-foreground">
                                {isClient ? t('My Invoices') : t('My Bills')}
                            </h1>
                            <p className="text-sm text-muted-foreground">
                                {invoices?.total || 0} {t('records')}
                            </p>
                        </div>
                    </div>
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            router.get(route('portal.invoices'), e.target.value ? { status: e.target.value } : {}, {
                                preserveState: true,
                            });
                        }}
                                ...
                                items.map((item) => (
                                    <tr key={item.id} className="transition hover:bg-muted/20">
                                        <td className="px-4 py-3 text-sm font-medium text-foreground">
                                            {item.invoice_id || item.bill_id || `#${item.id}`}
                                        </td>
                                        <td className="px-4 py-3 text-xs text-muted-foreground">
                                            {item.invoice_date || item.bill_date || '—'}
                                        </td>
                                        {isClient && (
                                            <td className="px-4 py-3 text-xs text-muted-foreground">
                                                {item.due_date || '—'}
                                        ...
                                        <td className="px-4 py-3">
                                            <span
                                                className={`inline-flex rounded-full px-2.5 py-0.5 text-[10px] font-medium ${statusColor[item.status] || 'bg-muted text-muted-foreground'}`}
                                            >
                                                {item.status}
                                            </span>
                                        </td>

    ...
    return (
        ...
        >
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-4">
                                <h1 className="text-2xl font-bold text-foreground">#{ticket.ticket_id} - {ticket.title}</h1>
                        ...
                        <div className="lg:w-80 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-foreground mt-1">{ticket.category?.name || '-'}</p>
                                </div>
                                <div>
                                    <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t('Created By')}</label>
                                    <p className="text-sm font-medium text-foreground mt-1">{ticket.creator?.name}</p>
                                </div>
                            </div>
                            <div>
                                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t('Created At')}</label>
                                <p className="text-sm font-medium text-foreground mt-1">{formatDate(ticket.created_at)}</p>
                            </div>
                            {ticket.assignedTo && (
                                <div>
                                    <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t('Assigned To')}</label>
                                    <p className="text-sm font-medium text-foreground mt-1">{ticket.assignedTo.name}</p>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            <Card className="flex flex-col" style={{ height: 'calc(100vh - 50px)' }}>
                <CardHeader className="border-b bg-muted/50/50 py-4">
                            ...
                            {replies.length === 0 ? (
                                <div className="flex items-center justify-center h-full">
                                    <div className="text-center">
                                        ...
                                        <h3 className="text-lg font-medium text-foreground mb-2">{t('No messages yet')}</h3>
                                        <p className="text-muted-foreground">{t('Start the conversation by sending a message below.')}</p>
                                    </div>
                                </div>
                            ) : (
                                <>

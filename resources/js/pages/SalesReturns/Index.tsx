    ...
    const tableColumns = [
        {
            ...
            render: (value: string, returnItem: SalesReturn) =>
                auth.user?.permissions?.includes('view-sales-return-invoices') ? (
                    <span className="text-foreground hover:text-foreground cursor-pointer" onClick={() => router.get(route('sales-returns.show', returnItem.id))}>{value}</span>
                ) : (
                    value
                )
        },
                                        ...
                                        <Card key={returnItem.id} className="border border-border flex flex-col">
                                            <div className="p-4 flex-1">
                                                <div className="flex items-center justify-between mb-3">
                                                    {auth.user?.permissions?.includes('view-sales-return-invoices') ? (
                                                        <h3 className="font-semibold text-base text-foreground hover:text-foreground cursor-pointer" onClick={() => router.get(route('sales-returns.show', returnItem.id))}>{returnItem.return_number}</h3>
                                                    ) : (
                                                ...
                                                <div className="space-y-2 mb-4">
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div>
                                                            ...
                                                            <p className="text-sm text-foreground truncate font-medium">{returnItem.customer?.name}</p>
                                                        </div>
                                                        <div>
                                                ...
                                                <div className="flex gap-1">
                                                    <TooltipProvider>
                                                        {returnItem.status === 'draft' && auth.user?.permissions?.includes('approve-sales-returns-invoices') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.post(route('sales-returns.approve', returnItem.id))} className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground">
                                                                        <CheckCircle className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                        ...
                                                        {returnItem.status === 'approved' && auth.user?.permissions?.includes('complete-sales-returns-invoices') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.post(route('sales-returns.complete', returnItem.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <Check className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                ...
                                                <div className="flex gap-1">
                                                    <TooltipProvider>
                                                        {auth.user?.permissions?.includes('view-sales-return-invoices') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('sales-returns.show', returnItem.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                                                        <Eye className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>

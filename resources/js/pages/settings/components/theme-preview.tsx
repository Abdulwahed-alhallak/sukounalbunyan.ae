...
export function ThemePreview({ 
  ...
  const systemIsDark = typeof document !== 'undefined' && (document.documentElement.classList.contains('dark') || document.body.classList.contains('dark'));
  ...
  return (
    <div 
      className={`border rounded-lg overflow-hidden text-xs transition-all duration-300 ${
        isDark ? 'bg-foreground text-background border-border' : 'bg-background text-foreground'
      } ${isRTL ? 'rtl' : 'ltr'}`}
      style={{
    ...
    >
      ...
      <div className={`px-3 py-2 border-b flex items-center justify-between ${
        isDark ? 'bg-card border-border' : 'bg-muted'
      }`}>
        ...
        <div className="flex items-center gap-1 order-2 rtl:order-1">
          <div className={`w-4 h-4 rounded ${
            isDark ? 'bg-muted' : 'bg-muted-foreground/20'
          }`}></div>
          <div className={`w-4 h-4 rounded ${
            isDark ? 'bg-muted' : 'bg-muted-foreground/20'
          }`}></div>
      ...
      <div className={`flex h-48 ${isRTL ? 'flex-row-reverse' : ''}`}>
        ...
        <div 
          className={`order-1 rtl:order-2 ${getSidebarStyles()} ${
            ...
            isDark && sidebarStyle === 'plain' ? 'bg-card border-border' : 
          ...
          <div className="space-y-2">
            ...
            {Array.from({ length: 5 }).map((_, i) => (
              <div 
                ...
                className={`w-full h-2 rounded ${
                  sidebarStyle === 'plain' 
                    ? (isDark ? 'bg-muted' : 'bg-muted-foreground/30')
                    : 'bg-card/30'
                }`}
              ></div>
            ))}

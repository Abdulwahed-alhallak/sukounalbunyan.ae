      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-3 md:col-span-2">
            ...
            {/* Attendance Deduction Auto Toggler */}
            <div className="space-y-3 md:col-span-2 p-4 border rounded-lg bg-foreground/5 mt-4">
              <div className="flex items-center justify-between gap-4">
                <div className="space-y-1">
                  <Label htmlFor="enable_attendance_deduction" className="text-base font-bold">{t('Automate Attendance Deductions')}</Label>
                  <p className="text-xs text-muted-foreground">{t('If enabled, running payroll will automatically deduct unrecorded absences and late minutes from an employee\'s salary. Recommended to be left ON if you are using Biometric Devices.')}</p>
                </div>
                <Switch
                  id="enable_attendance_deduction"
                  checked={settings.enable_attendance_deduction === 'on'}
                  onCheckedChange={(checked) => setSettings(prev => ({ ...prev, enable_attendance_deduction: checked ? 'on' : 'off' }))}
                  disabled={!canEdit}
                  className="scale-125"
                />
              </div>

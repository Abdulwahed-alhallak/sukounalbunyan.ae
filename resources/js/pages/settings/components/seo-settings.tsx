...
export default function SeoSettings({ userSettings, auth }: SeoSettingsProps) {
  ...
  const getDescriptionStatus = () => {
    ...
    if (length < 120) return { status: 'short', color: 'text-foreground', icon: AlertCircle };
    if (length <= 160) return { status: 'good', color: 'text-foreground', icon: CheckCircle2 };
    return { status: 'long', color: 'text-destructive', icon: AlertCircle };
  };
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-sm ${
              settings.metaTitle.length > 60 ? 'text-destructive' :
              settings.metaTitle.length > 50 ? 'text-foreground' : 'text-foreground'
            }`}>
                ...
                <CardContent className="pt-6">
                  ...
                  <div className="space-y-4">
                    <div className="border rounded-md p-3 bg-card">
                      <div className="text-xs text-foreground mb-1">example.com</div>
                      <div className="text-sm font-medium text-foreground hover:underline cursor-pointer line-clamp-1">
                    ...
                    <div className="border rounded-md p-3 bg-muted">
                      <div className="text-xs font-medium text-foreground mb-2">{t('SEO Tips')}</div>

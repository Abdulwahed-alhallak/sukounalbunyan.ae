...
conn.on('ready', () => {
    conn.exec('cd domains/sukounalbunyan.ae/public_html/backend && /opt/alt/php82/usr/bin/php artisan tinker --execute="echo json_encode(App\\Models\\Setting::where(\'key\', \'currencySymbol\')->get()->toArray());"', (err, stream) => {
        if (err) throw err;

...
class ProjectReportController extends Controller
{
    ...
    public function show($id)
    {
        if (Auth::user()->can('view-project-report')) {
            ...
            $usersData = $project->teamMembers->map(function ($user) use ($project) {
                ...
                foreach ($assignedTasks as $task) {
                    ...
                    if ($task->relationLoaded('timers')) {
                        ...
                        $totalSecondsLogged += \DionONE\Taskly\Models\TaskTimer::where('task_id', $task->id)
            ...
            // Project Expenses
            $totalExpenses = \DionONE\Taskly\Models\ProjectExpense::where('project_id', $project->id)->sum('amount');

...
use DionONE\Taskly\Events\DestroyProject;
use DionONE\Taskly\Events\DestroyProjectMilestone;
use DionONE\Taskly\Events\ProjectDeleteClient;
use DionONE\Taskly\Events\ProjectDeleteShareToClient;
use DionONE\Taskly\Events\ProjectInviteMember;
use DionONE\Taskly\Events\ProjectShareToClient;
use DionONE\Taskly\Events\UpdateProject;
use DionONE\Taskly\Events\UpdateProjectMilestone;
use DionONE\Taskly\Models\Project;
use DionONE\Taskly\Http\Requests\UpdateMilestoneRequest;
use DionONE\Taskly\Http\Requests\StoreMilestoneRequest;
use DionONE\Taskly\Http\Requests\StoreProjectRequest;
use DionONE\Taskly\Http\Requests\UpdateProjectRequest;
use DionONE\Taskly\Models\ActivityLog;
use DionONE\Taskly\Models\ProjectMilestone;
use DionONE\Taskly\Models\ProjectTask;
use DionONE\Taskly\Models\ProjectBug;
use DionONE\Taskly\Models\ProjectClient;
use DionONE\Taskly\Models\ProjectUser;
use DionONE\Taskly\Models\ProjectFile;
...
class ProjectController extends Controller
{
    ...
    public function show(Project $project)
    {
        if (Auth::user()->can('view-project') && $project->created_by == creatorId()) {
            ...
            $taskStages = \DionONE\Taskly\Models\TaskStage::where('created_by', creatorId())->orderBy('order')->get();
            $chartData = [];
            for ($i = 4; $i >= 0; $i--) {
            ...
            // Project Expenses
            $projectExpenses = \DionONE\Taskly\Models\ProjectExpense::where('project_id', $project->id)->orderBy('date', 'desc')->get();
    ...
    {
        if (Auth::user()->can('view-project') && $project->created_by == creatorId()) {
            $project->load(['teamMembers:id,name', 'tasks.assignedUser:id,name', 'tasks.milestone:id,title']);

            // Filter out tasks without start/end dates
            $tasks = $project->tasks->map(function ($task) {
                ...
                if ($task->duration && strpos($task->duration, ' - ') !== false) {
                    ...
                    try {
                        $startDate = \Carbon\Carbon::parse(trim($dateRange[0]))->format('Y-m-d');
                        $endDate = \Carbon\Carbon::parse(trim($dateRange[1]))->format('Y-m-d');
                    } catch (\Exception $e) {}
                }
                ...
                if ($task->assigned_to) {
                    ...
                    $assignedTo = \App\Models\User::whereIn('id', $userIds)->select('id', 'name')->get();
                }
                ...
                return [
                    ...
                    'start' => $startDate,
                    'end' => $endDate,
                    'progress' => $task->stage_id == 4 ? 100 : ($task->stage_id == 3 ? 75 : ($task->stage_id == 2 ? 50 : 0)), // Dummy logic for stage progress
                    'dependencies' => '',

...
namespace DionONE\Timesheet\Http\Controllers;
...
use DionONE\Timesheet\Http\Requests\StoreTimesheetRequest;
use DionONE\Timesheet\Http\Requests\UpdateTimesheetRequest;
use DionONE\Timesheet\Events\CreateTimesheet;
use DionONE\Timesheet\Events\UpdateTimesheet;
use DionONE\Timesheet\Events\DestroyTimesheet;
use App\Models\User;
...
class TimesheetController extends Controller
{
        ...
        if (Auth::user()->can('manage-timesheet')) {
            ...
            if (module_is_active('Taskly')) {
                $timesheets->getCollection()->transform(function ($timesheet) {
                    try {
                        if ($timesheet->project_id && class_exists('\DionONE\Taskly\Models\Project')) {
                            $project = \DionONE\Taskly\Models\Project::find($timesheet->project_id);
                            $timesheet->project_name = $project ? $project->name : 'N/A';
                        }
                        if ($timesheet->task_id && class_exists('\DionONE\Taskly\Models\ProjectTask')) {
                            $task = \DionONE\Taskly\Models\ProjectTask::find($timesheet->task_id);
            ...
            $projects = [];
            if (module_is_active('Taskly') && class_exists('\DionONE\Taskly\Models\Project')) {
                try {
                    if (Auth::user()->can('manage-any-timesheet')) {
                        $projects = \DionONE\Taskly\Models\Project::where('created_by', creatorId())
                            ->select('id', 'name')
                            ->get();
                    } else {
                        $projects = \DionONE\Taskly\Models\Project::whereHas('users', fn($q) => $q->where('user_id', Auth::id()))
                            ->select('id', 'name')
                            ->get();
                    }
                } catch (\Exception $e) {
    ...
    public function getAttendanceHours($userId, $date)
    {
        ...
        $attendance = \DionONE\Hrm\Models\Attendance::where('employee_id', $userId)
            ->whereDate('date', $date)
            ->first();

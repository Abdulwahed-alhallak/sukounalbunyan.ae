            ...
            <CardHeader className="pb-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">{ticket.email} • {formatDate(ticket.created_at)}</p>
                </div>
              </div>
            </CardHeader>
            <CardContent>
          ...
          {conversations && conversations?.map((conversion) => (
            <Card key={conversion.id} className={conversion.sender === 'admin' ? 'border-l-4 border-l-green-500' : 'border-l-4 border-l-gray-300'}>
              <CardHeader className="pb-3">
                <div className="flex items-center gap-3">
                  <Avatar className="h-10 w-10">
                    <AvatarFallback className={conversion.sender === 'admin' ? 'bg-muted/500 text-background' : 'bg-muted-foreground text-background'}>
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">{formatDate(conversion.created_at)}</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                ...
                {conversion.attachments && conversion.attachments.length > 0 && (
                  <div className={`mt-4 p-4 rounded-lg border ${
                    conversion.sender === 'admin' ? 'bg-muted/50 border-border' : 'bg-muted/50 border-border'
                  }`}>
                    <div className="flex items-center gap-2 mb-3">
                      <Paperclip className={`h-4 w-4 ${
                        conversion.sender === 'admin' ? 'text-foreground' : 'text-muted-foreground'
                      }`} />
                      <span className={`font-medium ${
                        conversion.sender === 'admin' ? 'text-foreground' : 'text-foreground'
                      }`}>{t('Attachments')}</span>
                    </div>
                    <div className="space-y-2">
              ...
              <form onSubmit={handleReplySubmit} className="space-y-4">
                <div>
                  ...
                  {replyErrors.description && (
                    <p className="text-sm text-destructive mt-1">{replyErrors.description}</p>
                  )}
                </div>
                ...
                <div className="flex justify-end">
                  <Button type="submit" disabled={replyProcessing || !replyData.description.trim()} className="bg-foreground hover:bg-foreground/80">
            ...
            <CardContent>
              <form onSubmit={handleNoteSubmit} className="space-y-4">
                <div>
                  ...
                  />
                  {noteErrors.note && (
                    <p className="text-sm text-destructive mt-1">{noteErrors.note}</p>
                  )}
                </div>
                <Button 
                  type="submit" 

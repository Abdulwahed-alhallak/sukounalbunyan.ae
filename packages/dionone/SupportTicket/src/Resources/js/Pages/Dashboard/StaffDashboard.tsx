        ...
        {!stats.canViewTickets ? (
          <Card className="bg-gradient-to-r from-muted/50 to-muted border-border">
            <CardContent className="p-8 text-center">
              ...
              <h2 className="text-xl font-bold text-destructive mb-2">{t('Access Restricted')}</h2>
              <p className="text-destructive">{t('You do not have permission to view support ticket data. Please contact your administrator.')}</p>
            </CardContent>
          </Card>
        ) : (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card className="bg-gradient-to-r from-muted/50 to-muted border-border cursor-pointer hover:shadow-lg transition-all duration-200" onClick={() => router.get(route('support-tickets.index'))}>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-foreground mt-1">{t('My tickets')}</p>
                </CardContent>
              </Card>

              <Card className="bg-gradient-to-r from-muted/50 to-muted border-border cursor-pointer hover:shadow-lg transition-all duration-200" onClick={() => router.get(route('support-tickets.index'))}>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium text-foreground">{t('In Progress')}</CardTitle>
                  <Clock className="h-6 w-6 text-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold text-foreground">{stats.openTickets}</div>
                  <p className="text-xs text-foreground mt-1">{t('Need attention')}</p>
                </CardContent>
              </Card>

              <Card className="bg-gradient-to-r from-muted/50 to-muted border-border cursor-pointer hover:shadow-lg transition-all duration-200" onClick={() => router.get(route('support-tickets.index'))}>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium text-foreground">{t('Resolved')}</CardTitle>

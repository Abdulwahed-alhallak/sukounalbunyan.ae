        ...
        return [{
            ...
            component: (
                <Label htmlFor="paypal-beauty-payment"
                    className="block border border-border rounded-lg p-4 hover:border-[#df9896] cursor-pointer transition-all duration-200">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-full overflow-hidden bg-card border">
...
export const lmsPayment = (data?: any) => {
    ...
    if (paypalEnabled === 'on') {
        return [{
            ...
            onFormSubmit: data?.onFormSubmit,
            component: (
                <div className="flex items-center space-x-3 p-3 border-2 border-border rounded-lg w-full hover:border-border transition-colors cursor-pointer">
...
export const parkingPayment = (data?: any) => {
    ...
    if (paypalEnabled === 'on') {
        return [{
            ...
            onFormSubmit: data?.onFormSubmit,
            component: (
                <div className="flex items-center space-x-3 p-3 border-2 border-border rounded-lg cursor-pointer hover:border-foreground transition-colors">
        ...
        return [{
            ...
            component: (
                <Label htmlFor="paypal-laundry-payment"
                    className="block border border-border rounded-lg p-4 hover:border-foreground cursor-pointer transition-all duration-200">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-full overflow-hidden bg-card border">
                                <img src={getPackageFavicon('Paypal')} alt="PayPal Logo" className="object-contain w-full h-full" />
        ...
        return [{
            ...
            component: (
                <label className="cursor-pointer">
                    ...
                    />
                    <div className={`p-4 border-2 rounded-lg transition-all hover:border-border flex items-center ${data?.selectedMethod === 'paypal' ? 'border-destructive bg-muted/50' : 'border-border'}`}>
                        <div className={`w-4 h-4 rounded-full border-2 mr-3 flex-shrink-0 ${data?.selectedMethod === 'paypal' ? 'border-destructive bg-muted/500' : 'border-border'}`}>
                            {data?.selectedMethod === 'paypal' && <div className="w-2 h-2 bg-card rounded-full m-auto mt-0.5"></div>}
                        </div>

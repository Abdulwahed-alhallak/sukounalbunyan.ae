        ...
        <div className="flex gap-1">
          <TooltipProvider>
            ...
            {auth.user?.permissions?.includes('view-formbuilder-responses') && (
              <Tooltip >
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="sm" onClick={() => router.visit(route('formbuilder.forms.responses', form.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
            ...
            {auth.user?.permissions?.includes('manage-formbuilder-conversions') && (
              <Tooltip >
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="sm" onClick={() => router.visit(route('formbuilder.forms.conversion', form.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
            ...
            {auth.user?.permissions?.includes('edit-formbuilder-form') && (
              <Tooltip >
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="sm" onClick={() => router.visit(route('formbuilder.forms.edit', form.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                  {forms.data?.map((form) => (
                    <Card key={form.id} className="p-0 hover:shadow-lg transition-all duration-200 relative overflow-hidden flex flex-col h-full min-w-0">
                      ...
                      <div className="p-4 bg-gradient-to-r from-primary/5 to-transparent border-b flex-shrink-0">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <h3 className="font-semibold text-sm text-foreground">{form.name}</h3>
                          </div>
                        </div>
                      </div>
                      <div className="p-4 flex-1 min-h-0">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="text-xs min-w-0">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs ${form.is_active ? 'bg-muted text-foreground' : 'bg-muted text-destructive'
                              }`}>
                      ...
                      <div className="flex justify-between gap-1 p-3 border-t bg-muted/50/50 flex-shrink-0 mt-auto">
                        <div className="flex gap-1">
                          <TooltipProvider>
                            <Tooltip >
                              <TooltipTrigger asChild>

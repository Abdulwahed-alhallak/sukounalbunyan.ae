              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="bg-muted/50 p-2 rounded">
                      <div className="font-medium text-foreground">{fields.filter(f => f.required).length}</div>
                  ...
                  {fieldTypes?.map((type) => {
                    ...
                    return (
                      <div
                        ...
                        className="relative flex flex-col items-center py-3 px-2 border border-border rounded-lg hover:border-foreground hover:bg-foreground/10 cursor-pointer transition-all group min-h-[70px]"
                      >
                        {count > 0 && (
                          <Badge className="absolute -top-1 -right-1 bg-foreground text-background text-xs h-5 w-5 rounded-full p-0 flex items-center justify-center font-medium">
                            {count}
                          </Badge>
                        )}
                        <div className="w-6 h-6 bg-muted rounded flex items-center justify-center mb-1 group-hover:bg-foreground/20">
                          <Icon className="w-3 h-3 text-muted-foreground group-hover:text-foreground" />
                        </div>
                        <div className="text-center">
                          <div className="font-medium text-xs text-foreground leading-tight">{t(type.label)}</div>
                        </div>
                      </div>
                    );
                  })}
          ...
          <form id="form-builder" onSubmit={handleSubmit}>
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                {/* Fields */}
                {fields.length === 0 ? (
                  <div className="text-center py-16 border-2 border-dashed border-border rounded-lg bg-muted/50">
                    <div className="w-20 h-20 bg-card rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm">
                      <Plus className="w-10 h-10 text-muted-foreground" />
                    </div>
                    <h3 className="text-xl font-semibold text-foreground mb-2">{t('Start Building Your Form')}</h3>
                    <p className="text-muted-foreground mb-6">{t('Click on any field type from the sidebar to add it to your form')}</p>
                    <div className="flex justify-center space-x-2">
                      <div className="px-3 py-1 bg-muted text-foreground rounded-full text-sm">{t('Easy Setup')}</div>
                      <div className="px-3 py-1 bg-muted text-foreground rounded-full text-sm">{t('Click to Add')}</div>
                    </div>
                  </div>
                ) : (
                    ...
                    {fields?.map((field, index) => {
                      ...
                      return (
                        <div key={field.id} className="group relative">
                          {/* Field Card */}
                          <div className="bg-card border-2 border-border rounded-xl p-6 hover:border-foreground/50 hover:shadow-lg transition-all duration-200">
                            ...
                            <div className="flex items-center justify-between mb-6">
                              <div className="flex items-center space-x-3">
                                <div className="w-10 h-10 bg-foreground/20 rounded-lg flex items-center justify-center">
                                  <FieldIcon className="w-5 h-5 text-foreground" />
                                </div>
                                <div>
                                  <div className="flex items-center space-x-2">
                                    <span className="text-sm font-semibold text-foreground">{t("Field")} {index + 1}</span>
                                    {field.required && (
                                  ...
                                  <span className="text-xs text-muted-foreground">{fieldTypeInfo?.label}</span>
                                </div>
                              </div>
                                  ...
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <Button
                                        ...
                                        disabled={index === 0}
                                        className="w-8 h-8 p-0 hover:bg-foreground/10"
                                      >
                            ...
                            <div className="grid grid-cols-12 gap-4 mb-4 align-center">
                              <div className="col-span-5">
                                <Label className="text-sm font-medium text-foreground mb-2 block">{t('Field Label')}</Label>
                                <Input
                                  value={field.label}
                              ...
                              <div className="col-span-5">
                                <Label className="text-sm font-medium text-foreground mb-2 block">{t('Placeholder Text')}</Label>
                                <Input
                            ...
                            {needsOptions(field.type) && (
                              <div className="mt-4 pt-4 border-t border-border">
                                <div className="flex items-center justify-between mb-4">
                                  <Label className="text-sm font-medium text-foreground">{t('Field Options')}</Label>
                                  <Button
                                    type="button"
                                    variant="outline"
                                    size="sm"

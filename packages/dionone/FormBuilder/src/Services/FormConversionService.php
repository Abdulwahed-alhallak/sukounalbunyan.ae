...
namespace DionONE\FormBuilder\Services;
...
class FormConversionService
{
    ...
    private function createProject(array $data): mixed
    {
        if ($this->user->can('create-project')) {
            try {
                $project = new \DionONE\Taskly\Models\Project();
    ...
    private function createLead(array $data): mixed
    {
        if ($this->user->can('create-leads')) {
            ...
            if (isset($data['pipeline_id'])) {
                $pipeline = \DionONE\Lead\Models\Pipeline::where('created_by', $this->created_by)
                    ->where('id', $data['pipeline_id'])
                    ->first();
            }

            if (!$pipeline) {
                $pipeline = \DionONE\Lead\Models\Pipeline::where('created_by', $this->created_by)->first();
            }
            ...
            $stage = \DionONE\Lead\Models\LeadStage::where('pipeline_id', $pipeline->id)->first();
            if (!$stage) {
                throw new \Exception('Please create a stage for this pipeline.');
            }

            $lead = new \DionONE\Lead\Models\Lead();
            $lead->name         = $data['name'] ?? '';
            ...
            foreach ($usrLeads as $usrLead) {
                \DionONE\Lead\Models\UserLead::firstOrCreate([
                    'user_id' => $usrLead,
                    'lead_id' => $lead->id,
                ]);
            }
    ...
    private function createDeal(array $data): mixed
    {
        if ($this->user->can('create-deals')) {
            ...
            // Get first stage from pipeline
            $stage = \DionONE\Lead\Models\DealStage::where('pipeline_id', $pipeline->id)->first();
            if (!$stage) {
                throw new \Exception('Please create a stage for this pipeline.');
            }

            $deal = new \DionONE\Lead\Models\Deal();
            $deal->name         = $data['name'] ?? '';
            ...
            if (isset($data['clients'])) {
                ...
                foreach ($clients as $clientId) {
                    if (!empty($clientId)) {
                        \DionONE\Lead\Models\ClientDeal::create([
                            'deal_id' => $deal->id,
                            'client_id' => $clientId,
                        ]);
                    }
            ...
            foreach ($usrDeals as $usrDeal) {
                \DionONE\Lead\Models\UserDeal::create([
                    'user_id' => $usrDeal,
                    'deal_id' => $deal->id,
                ]);
            }
    ...
    private function createSalesContact(array $data): mixed
    {
        if ($this->user->can('create-sales-contacts')) {
            $contact = new \DionONE\Sales\Models\SalesContact();
    ...
    private function createSalesOpportunity(array $data): mixed
    {
        if ($this->user->can('create-sales-opportunities')) {
            $opportunity = new \DionONE\Sales\Models\SalesOpportunity();
    ...
    private function createContract(array $data): mixed
    {
        if ($this->user->can('create-contracts')) {
            $contract = new \DionONE\Contract\Models\Contract();
    ...
    private function createBook(array $data): mixed
    {
        if ($this->user->can('create-internalknowledge-books')) {
            $book = new \DionONE\Internalknowledge\Models\InternalknowledgeBook();
    ...
    private function createArticle(array $data): mixed
    {
        if ($this->user->can('create-internalknowledge-articles')) {
            $article = new \DionONE\Internalknowledge\Models\InternalknowledgeArticle();
    ...
    private function createNote(array $data): mixed
    {
        if ($this->user->can('create-notes')) {
            $note = new \DionONE\Notes\Models\Note();
    ...
    private function createLocation(array $data): mixed
    {
        if ($this->user->can('create-cmms-locations')) {
            $location = new \DionONE\CMMS\Models\CmmsLocation();
    ...
    private function createMachine(array $data): mixed
    {
        if ($this->user->can('create-machines')) {
            $machine = new \DionONE\MachineRepairManagement\Models\Machine();

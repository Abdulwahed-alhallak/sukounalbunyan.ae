                    ...
                    <CardContent className="p-6">
                        <div className="flex items-start justify-between mb-6">
                            <div className="flex items-center gap-4">
                                <div className="w-16 h-16 bg-foreground/10 rounded-full flex items-center justify-center overflow-hidden">
                                ...
                                <div>
                                    ...
                                    <p className="text-muted-foreground">{(candidate as any).job_posting?.title || 'No Job Applied'}</p>
                                    <div className="flex items-center gap-2 mt-2">
                                        <Hash className="h-4 w-4 text-muted-foreground" />
                                        <span className="text-sm text-muted-foreground">{candidate.tracking_id || 'No Tracking ID'}</span>
                                    </div>
                                </div>
                            </div>
                            <span className={`px-2 py-1 rounded-full text-sm ${statusColors[candidate.status as keyof typeof statusColors] || 'bg-muted text-foreground'}`}>
                    ...
                    <Card className="shadow-sm">
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-sm font-medium text-muted-foreground">{t('Current Company')}</label>
                                    <p className="font-medium">{candidate.current_company || '-'}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-muted-foreground">{t('Current Position')}</label>
                                    <p className="font-medium">{candidate.current_position || '-'}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-muted-foreground">{t('Experience')}</label>
                                    <p className="font-medium">{candidate.experience_years || '0'} {t('years')}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-muted-foreground">{t('Notice Period')}</label>
                    ...
                    <Card className="shadow-sm">
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="text-sm font-medium text-muted-foreground">{t('Application Date')}</label>
                                    <p className="font-medium">{candidate.application_date ? formatDate(candidate.application_date) : '-'}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-muted-foreground">{t('Current Salary')}</label>
                                    <p className="font-medium">{candidate.current_salary ? formatCurrency(candidate.current_salary) : '-'}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-muted-foreground">{t('Expected Salary')}</label>
                                ...
                                {Object.entries(candidateAnswers)?.map(([questionId, answer]) => {
                                    ...
                                    return (
                                        <div key={questionId} className="space-y-2">
                                            ...
                                            <p className="font-medium bg-muted/50 p-3 rounded-md">{answer as string || '-'}</p>
                                        </div>
                                    );
                                })}
                            </div>

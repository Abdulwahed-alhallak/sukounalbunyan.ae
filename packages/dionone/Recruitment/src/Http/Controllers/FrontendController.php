...
use DionONE\Recruitment\Http\Requests\SubmitApplicationRequest;
use DionONE\Recruitment\Events\SubmitApplication;
    ...
    private function getUserIdFromRequest(Request $request)
    {
        try {
            ...
            return Inertia::render('Recruitment/Frontend/NotFound', [
                ...
                'brandSettings' => [
                    'logo' => $settings['logo_dark'] ?? 'packages/dionone/Recruitment/src/Resources/images/logo.png',
                    ...
                    'footerText' => $settings['footer_text'] ?? '© ' . date('Y') . ' DionONE. All rights reserved.'
                ]
            ]);
        } catch (\Exception $e) {
            $settings = RecruitmentSetting::where('created_by', $user->id)->pluck('value', 'key');

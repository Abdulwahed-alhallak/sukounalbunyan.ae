            ...
            case 'attachments':
                return (
                    <ModuleAttachments
                        ...
                        deleteRoute="lead.leads.attachments.destroy"
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        {videoHubButtons && videoHubButtons.length > 0 && (
                            <div className="flex items-center gap-2">
                                {videoHubButtons?.map((button: any, index: number) => (
                        ...
                        {spreadsheetButtons && spreadsheetButtons.length > 0 && (
                            <div className="flex items-center gap-2">
                                {spreadsheetButtons?.map((button: any, index: number) => (
                                    <div key={button.id || index}>{button.component}</div>
                                ))}
                            </div>
                        )}
                        {businessProcessMappingButtons && businessProcessMappingButtons.length > 0 && (
                            <div className="flex items-center gap-2">
                                {businessProcessMappingButtons?.map((button: any, index: number) => (
                                    <div key={button.id || index}>{button.component}</div>
                                ))}
                            </div>
                        )}

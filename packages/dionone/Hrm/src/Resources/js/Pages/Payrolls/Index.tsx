                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('run-payrolls') && payroll.is_payroll_paid !== 'paid' && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => runPayroll(payroll.id)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('view-payrolls') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('hrm.payrolls.show', payroll.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('edit-payrolls') && payroll.is_payroll_paid !== 'paid' && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', payroll)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
            ...
            <div className="space-y-8 animate-in fade-in duration-1000">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="premium-card p-6 bg-gradient-to-br from-muted/500/10 via-transparent to-transparent">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t('Total Liquidity')}</span>
                        </div>
                        <div className="space-y-1">
                            <h3 className="text-3xl font-black tracking-tight">
                                {formatCurrency(payrolls?.data?.reduce((acc, curr) => acc + (curr.total_net_pay || 0), 0) || 0)}
                            </h3>
                    ...
                    <div className="premium-card p-6 bg-gradient-to-br from-foreground/10 via-transparent to-transparent">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t('Processing')}</span>
                        </div>
                        <div className="space-y-1">
                            <h3 className="text-3xl font-black tracking-tight">
                    ...
                    <div className="premium-card p-6 bg-gradient-to-br from-foreground/10 via-transparent to-transparent">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t('Completed')}</span>
                        </div>
                        <div className="space-y-1">
                            <h3 className="text-3xl font-black tracking-tight">
                    ...
                    <div className="premium-card p-6 bg-gradient-to-br from-muted/500/10 via-transparent to-transparent">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t('Drafting')}</span>
                        </div>
                        <div className="space-y-1">
                            <h3 className="text-3xl font-black tracking-tight">
                ...
                <Card className="premium-card border-none bg-foreground/40 backdrop-blur-3xl overflow-hidden">
                    ...
                    <div className="p-6 border-b border-white/5 bg-card/5">
                        <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
                            <div className="w-full lg:flex-1 max-w-xl">
                                <SearchInput
                                    ...
                                    className="bg-background/20 border-white/10 placeholder:text-muted-foreground/30"
                                />
                            </div>
                        ...
                        {showFilters && (
                            <div className="mt-6 pt-6 border-t border-white/5 grid grid-cols-1 md:grid-cols-3 gap-6 animate-in slide-in-from-top duration-500">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t('Payout Frequency')}</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t('Register Status')}</label>
                                ...
                                <div className="flex items-end gap-3 pb-0.5">
                                    <Button onClick={handleFilter} className="bg-foreground hover:bg-foreground/80 font-black uppercase text-xs tracking-widest h-11 px-8 rounded-xl">{t('Sync Matrix')}</Button>
                                    <Button variant="ghost" onClick={clearFilters} className="text-muted-foreground hover:text-background font-black uppercase text-xs tracking-widest h-11">{t('Reset')}</Button>
                                </div>
                            </div>
                        )}
                    </div>

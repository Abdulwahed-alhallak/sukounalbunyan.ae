...
import { EditDocumentProps, EditDocumentFormData } from './types';
import { usePage } from '@inertiajs/react';

export default function EditDocument({ document, onSuccess }: EditDocumentProps) {
    const { documentcategories, users = [] } = usePage<any>().props;

    const { t } = useTranslation();
    const { data, setData, put, processing, errors } = useForm<EditDocumentFormData>({
    ...
    const titleAI = useFormFields('aiField', data, setData, errors, 'edit', 'title', 'Title', 'hrm', 'document');
    const descriptionAI = useFormFields('aiField', data, setData, errors, 'edit', 'description', 'Description', 'hrm', 'document');



    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('hrm.documents.update', document.id), {
            onSuccess: () => {
                onSuccess();
            }
        });

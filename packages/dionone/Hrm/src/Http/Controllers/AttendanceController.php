...
use DionONE\Hrm\Http\Requests\StoreAttendanceRequest;
use DionONE\Hrm\Http\Requests\UpdateAttendanceRequest;
use Illuminate\Routing\Controller;
use Illuminate\Support\Facades\Auth;
use Inertia\Inertia;
use DionONE\Hrm\Models\Employee;
use DionONE\Hrm\Models\Shift;
use DionONE\Hrm\Events\CreateAttendance;
use DionONE\Hrm\Events\UpdateAttendance;
use DionONE\Hrm\Events\DestroyAttendance;
use DionONE\Hrm\Models\LeaveApplication;
use DionONE\Hrm\Models\Holiday;
use DionONE\Hrm\Models\IpRestrict;
...
class AttendanceController extends Controller
{
    ...
    public function tracker()
    {
        if (Auth::user()->can('manage-attendances')) {
            ...
            $stats = [
                ...
                'absent' => $attendances->where('status', 'absent')->count() + ($totalEmployees - $attendances->count()),
                ...
                'late' => $attendances->filter(function($att) {
                     if ($att->clock_in && $att->shift && $att->shift->start_time) {
                         return \Carbon\Carbon::parse($att->clock_in)->format('H:i:s') > $att->shift->start_time;
                     }
                     return false;
                })->count(),
            ];

            return Inertia::render('Hrm/Attendances/Tracker', [
                'attendances' => $attendances,
                'stats' => $stats,
                'currentDate' => $date,
            ]);
    ...
    public function store(StoreAttendanceRequest $request)
    {
        if (Auth::user()->can('create-attendances')) {
            ...
            // Validate working day, leave, and holiday against the actual attendance date

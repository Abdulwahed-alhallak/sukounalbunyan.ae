...
use DionONE\Hrm\Http\Requests\StoreShiftRequest;
use DionONE\Hrm\Http\Requests\UpdateShiftRequest;
use Illuminate\Routing\Controller;
use Illuminate\Support\Facades\Auth;
use Inertia\Inertia;
use App\Models\User;
use DionONE\Hrm\Events\CreateShift;
use DionONE\Hrm\Events\UpdateShift;
use DionONE\Hrm\Events\DestroyShift;
...
class ShiftController extends Controller
{
    ...
    public function scheduler()
    {
        if (Auth::user()->can('manage-shifts')) {
            ...
            $employees = \DionONE\Hrm\Models\Employee::with(['user', 'shift'])
                ...
                ->map(function ($emp) {
                    return [
                        ...
                        'shift_name' => $emp->shift ? $emp->shift->shift_name : 'Unassigned',
                        'start_time' => $emp->shift ? $emp->shift->start_time : null,
                        'end_time' => $emp->shift ? $emp->shift->end_time : null,
                    ];
                });

            return Inertia::render('Hrm/Shifts/Scheduler', [
                'shifts' => $shifts,
                'employees' => $employees,
            ]);
        } else {

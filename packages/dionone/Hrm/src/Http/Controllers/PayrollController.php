...
use DionONE\Hrm\Models\Payroll;
use DionONE\Hrm\Http\Requests\StorePayrollRequest;
use DionONE\Hrm\Http\Requests\UpdatePayrollRequest;
use Illuminate\Routing\Controller;
use Illuminate\Support\Facades\Auth;
use Illuminate\Http\Request;
use Inertia\Inertia;
use DionONE\Hrm\Models\Allowance;
use DionONE\Hrm\Models\AllowanceType;
use DionONE\Hrm\Models\Attendance;
use DionONE\Hrm\Models\Deduction;
...
use DionONE\Hrm\Events\DestroyPayroll;
use DionONE\Hrm\Events\DestroySalarySlip;
...
class PayrollController extends Controller
{
    ...
    private function processEmployeePayroll($payroll, $employee, $workingDaysCount, $startDate, $endDate)
    {
        ...
        $attendanceData = $this->calculateAttendance($employee, $startDate, $endDate, $workingDaysCount, $leaveData);
        ...
        $lateDeductionAmount = $attendanceData['late_deduction_amount'];
        ...
        // Late Shortage Deductions
        if ($lateDeductionAmount > 0) {
            $deductionsBreakdown['Late & Shortage Deductions'] = round($lateDeductionAmount, 2);
    ...
    private function calculateAttendance($employee, $startDate, $endDate, $workingDaysCount = 0, $leaveData = null)
    {
        $attendances = Attendance::with('shift')->where('employee_id', $employee->user_id)
        ...
        // Auto Deduction Setting Check
        $enableDeduction = admin_setting('enable_attendance_deduction') === 'on';
        $lateDeductionAmount = 0;
        
        if ($enableDeduction) {
            // 1. Calculate missing attendance days as absent
            if ($workingDaysCount > 0 && $leaveData) {
                // Number of days employee was theoretically expected to be marked, minus what we actually have
                // Note: The precise working days algorithm was calculated in runPayroll based on calendar
                $totalLeaves = $leaveData['paid_leave_days'] + $leaveData['unpaid_leave_days'];
                $recordedDays = $presentDays + $halfDays + $absentDays + $totalLeaves;
                
                // If there are missed days in the biometric/attendance sheet, count them as absent
                if ($recordedDays < $workingDaysCount) {
                    $unrecordedMissingDays = $workingDaysCount - $recordedDays;
                    $absentDays += $unrecordedMissingDays;
                }
            }

            // 2. Calculate late minutes/hours from present days
            $hourlyRate = $employee->rate_per_hour ?? (($employee->basic_salary ?? 0) / max(1, $workingDaysCount) / 8); 
            $totalLateHours = 0;

            foreach ($attendances as $att) {
                if ($att->status !== 'absent' && $att->shift) {
                    // Try to determine shift required hours (assume 8 if not well defined)
                    $expectedHours = 8;
                    if ($att->shift->start_time && $att->shift->end_time) {
                        $sTime = \Carbon\Carbon::parse($att->shift->start_time);
                        $eTime = \Carbon\Carbon::parse($att->shift->end_time);
                        $expectedHours = max(1, $sTime->diffInHours($eTime) - ($att->shift->break_time ?? 0));
                    }
                    
                    if ($att->total_hour < $expectedHours) {
                        $shortage = $expectedHours - $att->total_hour;
                        // Avoid deducting for tiny margins (less than 15 mins)
                        if ($shortage > 0.25) {
                            $totalLateHours += $shortage;
                        }
                    }
                }
            }
            $lateDeductionAmount = $totalLateHours * $hourlyRate;
        }

        ...
        },
        {
            ...
            render: (value: number, contracttype: ContractType) => {
                ...
                if (totalCount === 0) {
                    return (
                        <div className="flex">
                            <span className="text-muted-foreground text-sm font-medium">{t('No contracts')}</span>
                        </div>
                    );
                }
                        ...
                        <div className="flex flex-wrap gap-1">
                            {displayContracts?.map((contract) => (
                                <Badge
                                    key={contract.id}
                                    variant="outline"
                                    className="text-xs bg-muted/50 border-border text-foreground hover:bg-muted"
                                >
                        ...
                        {auth.user?.permissions?.includes('edit-contract-types') && (contracttype.creator_id === auth.user?.id || contracttype.created_by === auth.user?.id) && (
                            <Tooltip >
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', contracttype)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">

                    ...
                    {(() => {
                        ...
                        return renewals.length > 0 ? (
                            <>
                                {renewalView === 'list' ? (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 p-4">
                                        {renewals?.map((renewal: any) => (
                                            <div key={renewal.id} className="group bg-card border border-border rounded-xl p-4 hover:shadow-lg hover:border-border transition-all duration-300 hover:-translate-y-1 cursor-pointer">


                                                {/* Value - Main Focus */}
                                                <div className="text-center mb-4 p-3 bg-gradient-to-r from-primary/5 to-primary/10 rounded-lg">
                                                    <p className="text-xs text-muted-foreground mb-1 font-medium">{t('Renewal Value')}</p>
                                                ...
                                                <div className="bg-muted/50 rounded-lg p-3 mb-4">
                                                    <div className="flex items-center justify-between text-xs">
                                                        <div className="text-center">
                                                            <p className="text-muted-foreground mb-1">{t('Start')}</p>
                                                            <p className="font-semibold text-foreground">{formatDate(renewal.start_date)}</p>
                                                        </div>
                                                        <div className="w-px h-8 bg-muted"></div>
                                                        <div className="text-center">
                                                            <p className="text-muted-foreground mb-1">{t('End')}</p>
                                                            <p className="font-semibold text-foreground">{formatDate(renewal.end_date)}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-2 mb-4 px-2">
                                                    ...
                                                    <span className="text-xs text-muted-foreground truncate font-medium">{renewal.creator?.name || '-'}</span>
                                                </div>

                                                {/* Notes Preview */}
                                                {renewal.notes && (
                                                    <div className="bg-muted/50 border border-border rounded-lg p-2 mb-4">
                                                        <p className="text-xs text-foreground line-clamp-2 leading-relaxed">{renewal.notes}</p>
                                                    </div>
                                                )}
                                                ...
                                                <div className="flex items-center justify-between pt-4 border-t border-border">
                                                    <div className="flex items-center gap-2 text-sm font-medium">
                                                        <div className={`w-2.5 h-2.5 rounded-full ${renewal.status === 'pending' ? 'bg-muted/500' :
                                                                renewal.status === 'approved' ? 'bg-muted/500' :
                                                                    renewal.status === 'active' ? 'bg-muted/500' :
                                                                        renewal.status === 'expired' ? 'bg-muted-foreground' :
                                                                            renewal.status === 'rejected' ? 'bg-muted/500' :
                                                                                'bg-muted-foreground'
                                                            }`} />
                            ...
                            <div className="text-center py-12">
                                <RefreshCw className="h-12 w-12 text-muted-foreground/60 mx-auto mb-3" />
                                <p className="text-muted-foreground text-sm">{renewalSearch ? t('No renewals found') : t('No renewals yet')}</p>
                                <p className="text-muted-foreground text-xs mt-1">{renewalSearch ? t('Try adjusting your search') : t('Create your first renewal')}</p>
                            </div>
                        );
                    })()}
                </CardContent>
                        ...
                        <div className="space-y-4 my-3">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="mt-1">
                                        <div className="flex items-center gap-2 text-sm font-medium">
                                            <div className={`w-2.5 h-2.5 rounded-full ${getSelectedRenewal().status === 'pending' ? 'bg-muted/500' :
                                                    getSelectedRenewal().status === 'approved' ? 'bg-muted/500' :
                                                        getSelectedRenewal().status === 'active' ? 'bg-muted/500' :
                                                            getSelectedRenewal().status === 'expired' ? 'bg-muted-foreground' :
                                                                getSelectedRenewal().status === 'rejected' ? 'bg-muted/500' :
                                                                    'bg-muted-foreground'
                                                }`} />

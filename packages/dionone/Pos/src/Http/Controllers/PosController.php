...
namespace DionONE\Pos\Http\Controllers;
...
use DionONE\Pos\Models\PosItem;
use DionONE\Pos\Http\Requests\StorePosRequest;
use DionONE\Pos\Events\CreatePosSale;
use DionONE\Pos\Models\PosPayment;
        ...
        $products = $query->get()
            ->map(function($product) {
                ...
                if ($product->tax_ids && is_array($product->tax_ids) && !empty($product->tax_ids)) {
                    $taxes = \DionONE\ProductService\Models\ProductServiceTax::whereIn('id', $product->tax_ids)
                        ->where('created_by', creatorId())

                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        ...
                        {auth.user?.permissions?.includes('edit-trainings') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', training)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                            ...
                                            <div className="p-4">
                                                <div className="flex items-center gap-3 mb-3">
                                                    ...
                                                    <div className="flex-1">
                                                        <h3 className="font-semibold text-base text-foreground">{training.title}</h3>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="space-y-3 mb-3">
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-2">{t('Training Type')}</p>
                                                            <p className="text-xs text-foreground truncate" title={training.trainingType?.name || trainingTypes.find(type => type.id === training.training_type_id)?.name}>
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-xs text-foreground truncate" title={training.location}>{training.location || '-'}</p>
                                                        </div>
                                                    </div>

                                                    <div className="grid grid-cols-2 gap-2">
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Trainer')}</p>
                                                            <p className="text-xs text-foreground truncate">{training.trainer?.name || '-'}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Branch')}</p>
                                                            <p className="text-xs text-foreground truncate">{training.branch?.branch_name || '-'}</p>
                                                        </div>
                                                    </div>

                                                    <div className="grid grid-cols-2 gap-2">
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Start Date')}</p>
                                                            <p className="text-xs text-foreground">{formatDate(training.start_date)}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('End Date')}</p>
                                                            <p className="text-xs text-foreground">{formatDate(training.end_date)}</p>
                                                        </div>                                                        
                                                    </div>

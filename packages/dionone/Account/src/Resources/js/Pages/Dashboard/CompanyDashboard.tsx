        ...
        >
            ...
            {stats && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                        ...
                        <Card className="bg-gradient-to-r from-muted/50 to-muted border-border">
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-sm font-medium text-foreground">{t('Total Customer Payment')}</CardTitle>
                                <ArrowDownCircle className="h-8 w-8 text-foreground opacity-80" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold text-foreground">{formatCurrency(stats.total_customer_payment || 0)}</div>
                                <p className="text-xs text-foreground opacity-80 mt-1">{t('Received payments')}</p>
                            </CardContent>
                        </Card>
                        <Card className="bg-gradient-to-r from-muted/50 to-muted border-border">
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-sm font-medium text-destructive">{t('Total Vendor Payment')}</CardTitle>
                                <ArrowUpCircle className="h-8 w-8 text-destructive opacity-80" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold text-destructive">{formatCurrency(stats.total_vendor_payment || 0)}</div>
                                <p className="text-xs text-destructive opacity-80 mt-1">{t('Paid to vendors')}</p>
                            </CardContent>
                        </Card>
                </div>
            )}
                                ...
                                <div className="max-h-80 overflow-y-auto space-y-3">
                                    {recentRevenues.slice(0, 5)?.map((revenue) => (
                                        <div key={revenue.id} className="flex justify-between items-center p-3 rounded-lg border">
                                            ...
                                            <div className="text-foreground font-bold">{formatCurrency(revenue.amount)}</div>
                                        </div>
                                    ))}
                                </div>
                            </CardContent>

...
export default function Index() {
    ...
    const getStatusBadgeClasses = (status: string) => {
        switch (status) {
            case 'draft': return 'px-2 py-1 rounded-full text-sm bg-muted text-foreground';
            case 'partial': return 'px-2 py-1 rounded-full text-sm bg-muted text-foreground';
            case 'approved': return 'px-2 py-1 rounded-full text-sm bg-muted text-foreground';
            case 'applied': return 'px-2 py-1 rounded-full text-sm bg-muted text-foreground';
            default: return 'px-2 py-1 rounded-full text-sm bg-muted text-foreground';
        }
    };
    ...
    const tableColumns = [
        {
            ...
            render: (value: string, debitNote: DebitNote) =>
                auth.user?.permissions?.includes('view-debit-notes') ? (
                    <span className="text-foreground hover:text-foreground cursor-pointer" onClick={() => router.get(route('account.debit-notes.show', debitNote.id))}>{value}</span>
                ) : (
                    value
                )
        },
            ...
            render: (value: any, debitNote: DebitNote) =>
                value?.return_number ? (
                    auth.user?.permissions?.includes('view-purchase-return-invoices') ? (
                        <span className="text-foreground hover:text-foreground cursor-pointer" onClick={() => router.get(route('purchase-returns.show', debitNote.purchase_return?.id))}>
                            {value.return_number}
                        </span>
                    ) : (
                        value.return_number
                ...
                {showFilters && (
                    <CardContent className="p-6 bg-muted/50/30 border-b">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Purchase Return')}</label>
                            ...
                            {debitNotes.data.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                    {debitNotes.data?.map((debitNote) => (
                                        <Card key={debitNote.id} className="border border-border flex flex-col">
                                            <div className="p-4 flex-1">
                                                <div className="mb-3">
                                                    {auth.user?.permissions?.includes('view-debit-notes') ? (
                                                        <h3 className="font-semibold text-base text-foreground hover:text-foreground cursor-pointer" onClick={() => router.get(route('account.debit-notes.show', debitNote.id))}>{debitNote.debit_note_number}</h3>
                                                    ) : (
                                                        <h3 className="font-semibold text-base text-foreground">{debitNote.debit_note_number}</h3>
                                                    )}
                                                </div>
                                                ...
                                                <div className="space-y-3 mb-3">
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-foreground truncate font-medium">{debitNote.vendor?.name}</p>
                                                    </div>
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Date')}</p>
                                                            <p className="text-xs text-foreground">{formatDate(debitNote.debit_note_date)}</p>
                                                        </div>
                                                        {debitNote.purchase_return && (
                                                            <div>
                                                                <p className="text-xs font-medium text-muted-foreground mb-1 text-end">{t('Purchase Return')}</p>
                                                                {auth.user?.permissions?.includes('view-purchase-return-invoices') ? (
                                                                    <p className="text-xs text-foreground cursor-pointer hover:text-foreground text-end" onClick={() => router.get(route('purchase-returns.show', debitNote.purchase_return.id))}>{debitNote.purchase_return.return_number}</p>
                                                                ) : (
                                                                    <p className="text-xs text-foreground text-end">{debitNote.purchase_return.return_number}</p>
                                                                )}
                                                            </div>
                                                        )}
                                                    </div>
                                                    <div className="bg-muted/50 rounded-lg p-3">
                                                        ...
                                                        <div className="border-t mt-2 pt-2">
                                                            <div className="flex justify-between items-center">
                                                                ...
                                                                <span className="text-lg font-bold text-foreground">{formatCurrency(parseFloat(debitNote.balance_amount.toString()))}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div>
                                                        <p className="text-xs font-medium text-muted-foreground mb-1">{t('Reason')}</p>
                                                        <p className="text-xs text-foreground line-clamp-2">{debitNote.reason}</p>
                                                    </div>
                                                    {debitNote.approved_by && (
                                                        <div>
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Approved By')}</p>
                                                            <p className="text-xs text-foreground">{debitNote.approved_by.name}</p>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>

                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('view-bank-accounts') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => setViewingItem(bankaccount)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('edit-bank-accounts') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', bankaccount)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                ...
                {showFilters && (
                    <CardContent className="p-6 bg-muted/50/30 border-b">
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Bank Name')}</label>
                                <Input
                            ...
                            {bankaccounts?.data?.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                    {bankaccounts?.data?.map((bankaccount) => (
                                        <Card key={bankaccount.id} className="border border-border flex flex-col">
                                            <div className="p-4 flex-1">
                                                <div className="flex items-center gap-3 mb-3">
                                                    <div className="w-12 h-12 bg-foreground rounded-lg border flex items-center justify-center">
                                                        <CreditCardIcon className="w-6 h-6 text-background" />
                                                    </div>
                                                    <div className="flex-1">
                                                        <h3 className="font-semibold text-base text-foreground">{bankaccount.account_number}</h3>
                                                    </div>
                                                </div>

                                                <div className="space-y-3 mb-3">
                                                    <div>
                                                        <p className="text-xs font-medium text-muted-foreground mb-2">{t('Account Name')}</p>
                                                        <p className="text-xs text-foreground truncate">{bankaccount.account_name || '-'}</p>
                                                    </div>
                                                    ...
                                                    {bankaccount.payment_gateway && (
                                                        <div className="flex justify-between">
                                                            <span className="text-sm text-muted-foreground">{t('Payment Gateway')}</span>
                                                            <span className="text-sm">{bankaccount.payment_gateway}</span>
                                                        </div>
                                                    )}
                                                    {bankaccount.gl_account && (
                                                        <div className="flex justify-between">
                                                            <span className="text-sm text-muted-foreground">{t('GL Account')}</span>

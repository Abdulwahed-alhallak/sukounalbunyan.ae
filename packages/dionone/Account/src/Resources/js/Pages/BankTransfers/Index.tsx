    ...
    const tableColumns = [
        {
            ...
            render: (value: string, transfer: BankTransfer) =>
                auth.user?.permissions?.includes('view-bank-transfers') ? (
                    <span className="text-foreground hover:text-foreground cursor-pointer" onClick={() => setViewingTransfer(transfer)}>{value}</span>
                ) : (
                    value
                )
        },
                ...
                <div className="flex gap-1">
                    ...
                    {transfer.status === 'pending' && auth.user?.permissions?.includes('process-bank-transfers') && (
                        ...
                        {transfer.status === 'pending' && auth.user?.permissions?.includes('edit-bank-transfers') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', transfer)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                ...
                {showFilters && (
                    <CardContent className="p-6 bg-muted/50/30 border-b">
                        <div className="flex flex-wrap items-end gap-4">
                            ...
                            <div className="flex-1 min-w-[200px]">
                                <label className="block text-sm font-medium text-foreground mb-2">{t('From Account')}</label>
                            ...
                            <div className="flex-1 min-w-[200px]">
                                <label className="block text-sm font-medium text-foreground mb-2">{t('To Account')}</label>

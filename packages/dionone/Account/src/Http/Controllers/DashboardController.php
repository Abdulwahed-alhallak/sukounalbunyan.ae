...
class DashboardController extends Controller
{
    ...
    private function vendorDashboard()
    {
        ...
        if (class_exists('\\DionONE\\Account\\Models\\DebitNote')) {
            $recentDebitNotes = \DionONE\Account\Models\DebitNote::where('vendor_id', $user->id)
                ->latest()
                ->limit(5)
                ->get()
                ->map(function($note) {
    ...
    private function clientDashboard()
    {
        ...
        if (class_exists('\\DionONE\\Account\\Models\\CreditNote')) {
            $recentCreditNotes = \DionONE\Account\Models\CreditNote::where('customer_id', $user->id)
                ->latest()
                ->limit(5)
                ->get()
                ->map(function($note) {

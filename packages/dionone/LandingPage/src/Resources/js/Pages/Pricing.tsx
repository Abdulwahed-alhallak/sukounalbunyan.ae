                    ...
                    {pricingSettings.show_pre_package === true && plans.length > 0 ? (
                        <div className="space-y-6 overflow-x-auto pt-6">
                            ...
                            <div className="grid gap-6" style={{ gridTemplateColumns: `300px repeat(${plans.length}, 280px)`, minWidth: `${300 + (plans.length * 280) + ((plans.length - 1) * 24)}px` }}>
                                {/* Features Header */}
                                <div className="bg-gradient-to-br from-muted/50 to-muted rounded-2xl p-6 border border-border sticky left-0 z-20">
                                    <div className="flex items-center justify-center space-x-3">
                                        <h3 className="text-xl font-bold text-foreground">{t("Features")}</h3>
                                    </div>
                                </div>
                                ...
                                {plans?.map((plan) => (
                                    <div 
                                        key={plan.id} 
                                        className={`relative rounded-2xl p-6 border-2 bg-card ${
                                    ...
                                    >
                                        {plan.id === mostPopularPlanId && plans.length > 1 && (
                                            <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                                                <div 
                                                    className="text-background px-4 py-2 text-sm font-bold shadow-lg rounded-md whitespace-nowrap"
                                        ...
                                        <div className="text-center space-y-4">
                                            ...
                                            <div className="space-y-3 py-4">
                                                ...
                                                {plan.trial && (
                                                    <div className="flex items-center space-x-2">
                                                        <div className="w-2 h-2 rounded-full bg-muted/500 flex-shrink-0"></div>
                                                        <span className="text-sm font-medium text-foreground">
                            ...
                            <div className="space-y-4">
                                <div className="grid gap-6" style={{ gridTemplateColumns: `300px repeat(${plans.length}, 280px)`, minWidth: `${300 + (plans.length * 280) + ((plans.length - 1) * 24)}px` }}>
                                    {/* All Modules Card */}
                                    <div className="bg-card rounded-2xl p-6 border border-border sticky left-0 z-20">
                                        <div className="space-y-3">
                                            <div className="flex items-center justify-center py-2 h-10 border-b border-border mb-3">
                                    ...
                                    {plans?.map((plan) => {
                                        ...
                                        return (
                                        <div key={plan.id} className="bg-card rounded-2xl p-6 border border-border">
                                            <div className="space-y-3">
                                                <div className="flex items-center justify-center py-2 h-10 border-b border-border mb-3">
                                                    <span className="text-foreground font-semibold text-sm">
                                                ...
                                                <div className="pt-4 border-t space-y-2">
                                                    <button 
                                                        className="w-full py-2 px-4 rounded-md text-background font-medium transition-colors"
                                                        style={{ backgroundColor: colors.primary }}
                                                        onClick={() => {
                        ...
                        <div className="text-center py-16">
                            ...
                            <h3 className="text-xl font-semibold text-foreground mb-2">{t("No Plans Available")}</h3>
                            <p className="text-muted-foreground">{pricingSettings.empty_message || t('Check back later for new pricing plans.')}</p>
                        </div>
                    )}
                </div>
            </main>

...
const FEATURES_VARIANTS = {
    ...
    features4: {
        ...
        subtitle: 'text-lg text-muted-foreground/60 mb-16',
...
export default function Features({ settings }: FeaturesProps) {
    ...
    const renderFeature = (feature: any, index: number) => {
        ...
        if (config.layout === 'grid') {
            return (
                <div key={index} className="text-center p-6 hover:bg-muted/50 rounded-lg transition-colors">
        ...
        if (config.layout === 'cards') {
            return (
                <div key={index} className="bg-card p-8 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
        ...
        if (config.layout === 'split') {
            return (
                <div key={index} className="flex items-center space-x-4 p-4 bg-card rounded-lg">
                    ...
                    <div>
                        <h3 className="text-lg font-semibold text-background mb-1">{feature.title}</h3>
                        <p className="text-muted-foreground/60 text-sm">{feature.description}</p>
                    </div>
                </div>
            );
        }
        ...
        if (config.layout === 'carousel') {
            return (
                <div key={index} className="flex-shrink-0 w-80 mr-6 group">
                    <div className="bg-card p-8 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 border border-border hover:border-border transform hover:-translate-y-2 h-full">
                        ...
                        <h3 className="text-xl font-bold text-foreground mb-4 group-hover:text-foreground transition-colors">{feature.title}</h3>
            ...
            <section className={config.section}>
                <div className={config.container}>
                    ...
                    <div className="relative">
                        ...
                        <button
                            onClick={prevSlide}
                            className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-card/90 hover:bg-card text-foreground p-3 rounded-full shadow-xl transition-all hover:scale-110 z-10"
                        >
                            <ChevronLeft className="w-6 h-6" />
                        </button>
                        <button
                            onClick={nextSlide}
                            className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-card/90 hover:bg-card text-foreground p-3 rounded-full shadow-xl transition-all hover:scale-110 z-10"
                        >

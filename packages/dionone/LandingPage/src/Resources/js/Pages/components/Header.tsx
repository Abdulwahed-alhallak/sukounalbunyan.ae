...
const HEADER_VARIANTS = {
    header1: {
        nav: 'bg-card border-b border-border sticky top-0 z-50',
        ...
        mobile: 'md:hidden text-muted-foreground p-2 transition-colors',
        mobileMenu: 'md:hidden bg-card border-t'
    },
    header2: {
        nav: 'bg-card border-b border-border sticky top-0 z-50 shadow-sm',
        container: 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8',
        wrapper: 'flex flex-col items-center py-6 space-y-6',
        logo: 'text-3xl font-bold',
        desktop: 'flex items-center space-x-2 bg-muted/50 px-6 py-3 rounded-full',
        mobile: 'md:hidden text-muted-foreground p-2 transition-colors absolute top-4 right-4 hover:bg-muted rounded-lg',
        mobileMenu: 'md:hidden bg-card border-t w-full shadow-lg'
    },
    header3: {
        nav: 'bg-card/95 backdrop-blur-md border-b border-border sticky top-0 z-50',
        container: 'max-w-6xl mx-auto px-6 sm:px-8 lg:px-10',
        wrapper: 'flex justify-between items-center h-14 py-2',
        logo: 'text-xl font-bold',
        desktop: 'hidden md:flex items-center space-x-2',
        mobile: 'md:hidden text-muted-foreground p-2 transition-colors hover:bg-muted rounded-md',
        mobileMenu: 'md:hidden bg-card/95 backdrop-blur-md border-t'
    },
    header4: {
        nav: 'bg-foreground/20 backdrop-blur-md absolute top-0 left-0 right-0 z-50 border-b border-white/10',
        container: 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8',
        wrapper: 'flex justify-between items-center h-20 py-4',
        logo: 'text-2xl font-bold text-background drop-shadow-lg',
        desktop: 'hidden md:flex items-center space-x-2',
        mobile: 'md:hidden text-background p-2 transition-colors hover:bg-card/10 rounded-lg',
        mobileMenu: 'md:hidden bg-foreground/90 backdrop-blur-md border-t border-white/10'
    },
    header5: {
...
export default function Header({ settings }: HeaderProps) {
    ...
    const companyName = sectionData.company_name || settings?.company_name || 'DionCreative';
    ...
    const renderNavItems = (isMobile = false) => {
        ...
        const textColor = isTransparentOrGradient ? 'text-background' : 'text-muted-foreground';
        const hoverBg = variant === 'header2' ? 'hover:bg-card hover:shadow-sm' : variant === 'header3' ? 'hover:bg-muted/50' : isTransparentOrGradient ? 'hover:bg-card/10' : 'hover:bg-muted/50';
        ...
        if (isAuthenticated) {
            return (
                <button 
                    ...
                    className={`text-background rounded-md font-medium transition-colors ${
                            ...
                            <Link 
                                ...
                                className={`px-4 py-2 text-sm font-medium transition-all duration-200 rounded-lg ${
                                    variant === 'header4' || variant === 'header5' 
                                        ? 'text-background hover:bg-card/10' 
                                        : variant === 'header2' 

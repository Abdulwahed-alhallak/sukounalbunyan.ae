...
export function LandingPreview({ settings }: LandingPreviewProps) {
  ...
  const renderMiniSection = (sectionKey: string) => {
    ...
    switch (sectionKey) {
      case 'header':
        return (
          <div key={sectionKey} className="flex justify-between items-center p-3 bg-card border-b shadow-sm">
            <div className="text-sm font-bold" style={{ color: colors.primary }}>
              {sectionData.company_name || settings?.company_name || 'DionCreative'}
            </div>
            <div className="text-xs text-background px-3 py-1 rounded-full shadow-sm transition-colors" style={{ backgroundColor: colors.primary }}>
      ...
      case 'hero':
        return (
          <div key={sectionKey} className="p-4 text-background text-center relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${colors.primary}, ${colors.secondary}, ${colors.accent})` }}>
            ...
            <div className="relative z-10">
              ...
              <div className="flex gap-2 justify-center">
                <div className="text-xs bg-card px-3 py-1.5 rounded-full font-medium shadow-lg hover:shadow-xl transition-shadow" style={{ color: colors.primary }}>
                  {sectionData.primary_button_text || t('Start Trial')}
                </div>
                <div className="text-xs border border-white/50 px-3 py-1.5 rounded-full backdrop-blur-sm hover:bg-card/10 transition-colors">
      ...
      case 'stats':
        return (
          <div key={sectionKey} className="p-4 text-background" style={{ background: `linear-gradient(to right, ${colors.primary}, ${colors.secondary})` }}>
            <div className="grid grid-cols-2 gap-3 text-center">
              <div className="bg-card/10 rounded-lg p-2 backdrop-blur-sm">
      ...
      case 'features':
        return (
          <div key={sectionKey} className="p-4 bg-muted/50">
            <div className="text-sm font-bold text-center mb-3 text-foreground">
              {sectionData.title || t('Powerful Features')}
            </div>
            <div className="grid grid-cols-2 gap-2">
              {(sectionData.features || [{}, {}, {}, {}]).slice(0, 4)?.map((feature: any, i: number) => (
                <div key={i} className="bg-card p-2 rounded-lg shadow-sm border hover:shadow-md transition-shadow">
                  ...
                  <div className="text-xs text-center font-medium text-foreground">{feature.title?.substring(0, 10) || `Feature ${i+1}`}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      case 'modules':
        return (
          <div key={sectionKey} className="p-2 bg-card">
            ...
            <div className="flex gap-1 justify-center">
              {['ERP', 'CRM', 'HRM', 'POS']?.map((module, i) => (
                <div key={i} className="text-xs bg-muted px-1 py-0.5 rounded">
                  {module}
                </div>
              ))}
            </div>
      ...
      case 'benefits':
        return (
          <div key={sectionKey} className="p-2 bg-muted/50">
            <div className="text-xs font-bold text-center mb-1">
            ...
            <div className="space-y-1">
              {Array.from({ length: 3 })?.map((_, i) => (
                <div key={i} className="flex items-center gap-1">
                  <div className="w-1 h-1 bg-muted/500 rounded-full"></div>
                  <div className="text-xs text-muted-foreground">{t('Benefit')} {i + 1}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      case 'cta':
        return (
          <div key={sectionKey} className="p-2 text-background text-center" style={{ backgroundColor: colors.primary }}>
            <div className="text-xs font-bold mb-1">
              {sectionData.title || t('Ready to Transform?')}
            </div>
            <div className="flex gap-1 justify-center">
              <div className="text-xs bg-card px-2 py-1 rounded" style={{ color: colors.primary }}>
      ...
      case 'footer':
        return (
          <div key={sectionKey} className="p-4 bg-gradient-to-r from-foreground to-card text-background">
            <div className="grid grid-cols-1 gap-3 text-xs">
              <div className="text-center border-b border-border pb-2">
                <div className="font-bold text-sm" style={{ color: colors.accent }}>
                  {settings?.company_name || t('DionCreative')}
                </div>
  ...
  return (
    <div className="border rounded-lg overflow-hidden bg-card shadow-lg">
      <div className="px-3 py-2 border-b" style={{ background: `linear-gradient(to right, ${colors.primary}, ${colors.secondary})` }}>
        <div className="text-sm font-semibold text-background flex items-center gap-2">
      ...
      <div className="relative">
        <div className="absolute top-2 right-2 z-10">
          <div className="bg-foreground/20 backdrop-blur-sm rounded-full px-2 py-1">
            <div className="text-xs text-background font-medium">{t('Mobile View')}</div>
          </div>
        </div>

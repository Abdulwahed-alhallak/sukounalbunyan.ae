...
const STATS_VARIANTS = {
    stats1: {
        ...
        statValue: 'text-4xl md:text-5xl font-bold text-background mb-2',
        statLabel: 'text-foreground text-sm md:text-base',
        layout: 'colored'
    },
    stats2: {
        ...
        statLabel: 'text-muted-foreground text-sm md:text-base font-medium',
        layout: 'cards'
    },
    stats3: {
        section: 'bg-card py-16',
        container: 'max-w-6xl mx-auto px-4 sm:px-6 lg:px-8',
        grid: 'grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12',
        statValue: 'text-3xl md:text-4xl font-bold text-foreground mb-2',
        statLabel: 'text-muted-foreground text-sm md:text-base font-medium',
        layout: 'minimal'
    },
    stats4: {
        ...
        statValue: 'text-xl md:text-2xl font-bold text-background',
        statLabel: 'text-muted-foreground/60 text-xs md:text-sm font-medium',
        layout: 'circular'
    },
    stats5: {
        section: 'py-20',
        ...
        grid: 'grid grid-cols-2 md:grid-cols-4 gap-8 text-center',
        statValue: 'text-4xl md:text-5xl font-bold text-background mb-2',
        statLabel: 'text-background text-sm md:text-base opacity-90',
        layout: 'gradient'
    }
};
...
export default function Stats({ settings }: StatsProps) {
    ...
    const renderStat = (stat: any, index: number) => {
        if (config.layout === 'cards') {
            return (
                <div key={index} className="group bg-card p-8 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 text-center border border-border hover:border-border transform hover:-translate-y-1">

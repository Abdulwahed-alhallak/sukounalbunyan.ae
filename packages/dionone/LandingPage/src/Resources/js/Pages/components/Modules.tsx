...
const MODULES_VARIANTS = {
    modules1: {
        ...
        subtitle: 'text-lg text-muted-foreground mb-16 text-center max-w-3xl mx-auto',
        layout: 'tabs'
    },
    modules2: {
...
export default function Modules({ settings }: ModulesProps) {
    ...
    const renderTabs = () => (
        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div>
                    <h3 className="text-2xl font-bold text-foreground mb-4">{modules[activeTab]?.title}</h3>
                    <p className="text-muted-foreground text-lg">{modules[activeTab]?.description}</p>
                </div>
                <div className="bg-muted rounded-lg h-80 flex items-center justify-center overflow-hidden shadow-[0px_2px_4px_0px_rgba(14,30,37,0.12),0px_2px_16px_0px_rgba(14,30,37,0.32)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {modules?.map((module: any, index: number) => (
                <div key={index} className="bg-card p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
                    <div className="h-48 bg-muted rounded-lg mb-6 flex items-center justify-center overflow-hidden">
                        {module.image ? (
    ...
    const renderAccordion = () => (
        <div className="space-y-6 max-w-4xl mx-auto">
            {modules?.map((module: any, index: number) => (
                <div key={index} className="bg-card border border-border rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
                    <button
                    ...
                    >
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <span className="font-bold text-foreground text-lg">{module.label}</span>
                                <p className="text-sm text-muted-foreground mt-1">{module.description?.substring(0, 60)}...</p>
                            </div>
                        </div>
                        <ChevronDown className={`h-6 w-6 text-muted-foreground transition-transform duration-300 ${openAccordion === index ? 'rotate-180' : ''}`} style={{ color: openAccordion === index ? colors.primary : undefined }} />
                    </button>
                    {openAccordion === index && (
                        <div className="px-8 pb-8 bg-muted/50">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center pt-6">
                                ...
                                <div className="h-64 bg-muted rounded-xl flex items-center justify-center overflow-hidden shadow-inner">
                                    {module.image ? (
                                        <img src={module.image.startsWith('http') ? module.image : getImagePath(module.image)} alt={module.title} className="w-full h-full object-cover" />
                                    ) : (
                                        <div className="text-center">
                                            <Monitor className="h-16 w-16 text-muted-foreground mx-auto mb-2" />
                                            <p className="text-muted-foreground text-sm">{t('Module Preview')}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
    ...
    const renderSlider = () => (
        <div className="relative">
            <div className="overflow-hidden rounded-xl">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center bg-card p-12 rounded-xl">
                    <div>
                        <h3 className="text-2xl font-bold text-background mb-4">{modules[currentSlide]?.title}</h3>
                        <p className="text-muted-foreground/60 text-lg mb-6">{modules[currentSlide]?.description}</p>
                            ...
                            {modules?.map((_: any, index: number) => (
                                <button
                                    ...
                                    className={`w-3 h-3 rounded-full transition-colors ${
                                        currentSlide === index ? 'bg-card' : 'bg-muted'
                                    }`}
                                />
                            ))}
                        </div>
    ...
    const renderGrid = () => (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {modules?.map((module: any, index: number) => (
                <div key={index} className="bg-card rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-border hover:border-border group">
                    <div className="p-8">
                        <div className="flex items-start space-x-6">
                            <div className="flex-shrink-0 relative">
                                ...
                                <div className="absolute -top-2 -right-2 w-6 h-6 rounded-full flex items-center justify-center text-background text-xs font-bold" style={{ backgroundColor: colors.primary }}>
                                    {index + 1}
                                </div>
                            </div>
                            <div className="flex-1">
                                <h3 className="text-xl font-bold text-foreground mb-3 group-hover:text-foreground transition-colors">{module.title}</h3>

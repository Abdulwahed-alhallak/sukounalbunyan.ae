...
const BENEFITS_VARIANTS = {
    benefits1: {
        ...
        title: 'text-3xl md:text-4xl font-bold text-foreground mb-12 text-center',
        layout: 'accordion'
    },
    benefits2: {
    ...
    benefits4: {
        ...
        title: 'text-3xl md:text-4xl font-bold text-background mb-12 text-center',
        layout: 'timeline'
    },
    benefits5: {
        section: 'bg-card py-20',
...
export default function Benefits({ settings }: BenefitsProps) {
    ...
    const title = sectionData.title || 'Why Choose DionCreative?';
            ...
            {benefits?.map((benefit: any, index: number) => (
                <div key={index} className="border border-border rounded-lg overflow-hidden">
                    ...
                    >
                        ...
                        <ChevronDown className={`h-5 w-5 text-muted-foreground transition-transform ${openAccordion === index ? 'rotate-180' : ''}`} />
                    </button>
                    {openAccordion === index && (
    ...
    const renderList = () => (
        <div className="space-y-6">
            {benefits?.map((benefit: any, index: number) => (
                <div key={index} className="flex items-start space-x-4 p-6 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
                ...
                {benefits?.map((benefit: any, index: number) => (
                    <div key={index} className="relative flex items-start space-x-6">
                        ...
                        <div className="bg-card p-6 rounded-lg flex-1">
                            <h3 className="text-xl font-semibold text-background mb-3">{benefit.title}</h3>
                            <p className="text-muted-foreground/60">{benefit.description}</p>
                        </div>
                    </div>
                ))}
            </div>
        ...
        <div>
            ...
            <div className="text-center">
                <div className="bg-muted/50 p-8 rounded-lg">
                    <h3 className="text-2xl font-semibold text-foreground mb-4">{benefits[activeTab]?.title}</h3>
                    <p className="text-lg text-muted-foreground">{benefits[activeTab]?.description}</p>
                </div>
            </div>
        </div>
    );

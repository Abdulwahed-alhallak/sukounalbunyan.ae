...
const CTA_VARIANTS = {
    cta1: {
        ...
        subtitle: 'text-xl text-background/90 mb-8',
    ...
    cta4: {
        ...
        title: 'text-4xl md:text-5xl font-bold text-background mb-6',
        subtitle: 'text-xl text-background/90 mb-10',
        buttons: 'flex flex-col sm:flex-row gap-6 justify-center',
        layout: 'gradient'
    },
    cta5: {
        section: 'bg-gradient-to-b from-muted/50 to-white py-20 border-t border-border',
        container: 'max-w-4xl mx-auto px-6 sm:px-8 lg:px-12 text-center',
        title: 'text-2xl md:text-3xl lg:text-4xl font-bold text-foreground mb-6',
        subtitle: 'text-base md:text-lg text-muted-foreground mb-8',
...
export default function CTA({ settings }: CTAProps) {
    ...
    const subtitle = sectionData.subtitle || 'Join thousands of businesses already using DionCreative to streamline their operations.';
    ...
    const renderButtons = () => {
        ...
        return (
            <div className={config.buttons}>
                <a 
                    href={primaryLink}
                    className={`inline-flex items-center justify-center text-background px-8 py-3 rounded-lg font-semibold transition-all duration-300 hover:scale-105 hover:shadow-lg ${
                        config.layout === 'minimal' 
                            ...
                            : config.layout === 'card'
                                ...
                                : 'text-lg'
                    } ${config.layout === 'split' ? 'shadow-lg hover:shadow-xl' : 'bg-card/20 hover:bg-card/30 backdrop-blur-sm'}`}
                    style={{ 
                ...
                <a 
                    ...
                    className={`inline-flex items-center justify-center border-2 px-8 py-3 rounded-lg font-semibold transition-all duration-300 hover:scale-105 ${
                        config.layout === 'minimal' 
                            ? 'text-base px-8 py-3.5 border-border text-foreground hover:bg-muted/50 hover:border-border rounded-lg shadow-sm hover:shadow-md transform hover:-translate-y-0.5' 
                            : config.layout === 'card'
                                ? 'text-base px-10 py-4 border-border text-foreground hover:bg-muted/50 hover:border-border rounded-xl shadow-sm hover:shadow-lg transform hover:-translate-y-1'
                                : config.layout === 'split'
                                    ? 'border-border text-foreground hover:bg-muted/50 hover:text-foreground hover:border-border shadow-md hover:shadow-lg'
                                    : 'border-white text-background hover:bg-card hover:text-foreground backdrop-blur-sm'
                    }`}
                >
                    {secondaryButton}
                </a>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="relative overflow-hidden rounded-xl shadow-2xl">
                            {sectionData.image ? (
                                ...
                                <div className="bg-gradient-to-br from-muted to-muted h-80 flex items-center justify-center">
                                    <div className="text-center">
                                        ...
                                        <p className="text-muted-foreground font-medium">{t('Upload CTA Image')}</p>
                                    </div>
                                </div>
                            )}
                        </div>
    ...
    if (config.layout === 'card') {
        return (
            <section className={config.section}>
                <div className={config.container}>
                    <div className="bg-card p-8 md:p-12 lg:p-16 rounded-2xl shadow-xl text-center border border-border/50 relative overflow-hidden">
                        <div className="absolute inset-0 bg-gradient-to-br from-muted/50/80 via-white to-muted/50/80"></div>
                        <div className="relative z-10 space-y-8">
                            <div className="space-y-6">
                                ...
                                <div>
                                    <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-foreground mb-4 leading-tight">{title}</h2>
                                    <p className="text-base md:text-lg text-muted-foreground max-w-xl mx-auto leading-relaxed">{subtitle}</p>
                                </div>
                            </div>
                            <div className="pt-4">
                                {renderButtons()}

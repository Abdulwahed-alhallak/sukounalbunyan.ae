    ...
    const renderSlider = () => {
        ...
        return (
            <div className="relative max-w-5xl mx-auto">
                <div className="bg-gradient-to-br from-foreground to-card p-10 rounded-2xl shadow-2xl">
                    <div className="text-center mb-8">
                        <div className="inline-flex items-center space-x-2 bg-card/10 rounded-full px-4 py-2 mb-4">
                            <Package className="h-4 w-4 text-background" />
                            <span className="text-background text-sm font-medium">{currentSlide + 1} of {packages.length}</span>
                        </div>
                        <h3 className="text-3xl font-bold text-background mb-3">{currentPackage.name}</h3>
                        <p className="text-muted-foreground/60 text-lg max-w-2xl mx-auto">{currentPackage.description}</p>
                    </div>
                            ...
                            {packages?.map((_: any, index: number) => (
                                <button
                                    ...
                                    className={`transition-all duration-300 rounded-full ${
                                        currentSlide === index 
                                            ? 'w-8 h-3 bg-card' 
                                            : 'w-3 h-3 bg-card/40 hover:bg-card/60'
                                    }`}
                                />
                            ))}
                        </div>

...
const DEDICATION_VARIANTS = {
    dedication1: {
        ...
        subtitle: 'text-lg text-muted-foreground max-w-3xl mx-auto text-center',
        layout: 'alternating'
    },
    dedication2: {
    ...
    const renderTabs = () => (
        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div>
                    <h3 className="text-2xl font-bold text-foreground mb-4">{subSections[activeTab]?.title}</h3>
                    <p className="text-muted-foreground mb-6">{subSections[activeTab]?.description}</p>
                    <div className="space-y-3">
            ...
            <div className="space-y-16">
                {subSections?.map((section: any, index: number) => (
                    <div key={index} className="relative flex items-start space-x-8">
                        ...
                        <div className="flex-1 bg-gradient-to-br from-card to-foreground p-8 rounded-xl shadow-2xl">
                            <h3 className="text-2xl font-bold text-background mb-4">{section.title}</h3>
                            <p className="text-muted-foreground/60 mb-6 leading-relaxed">{section.description}</p>
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                                <div className="space-y-3">
                                    {section.keyPoints?.map((point: string, pointIndex: number) => (
                                        <div key={pointIndex} className="flex items-center">
                                            <CheckCircle className="h-5 w-5 text-muted-foreground mr-3 flex-shrink-0" />
                                            <span className="text-muted-foreground/60">{point}</span>
                                        </div>
                                    ))}
                                </div>
        ...
        return (
            <div className="relative">
                <div className="overflow-hidden rounded-xl">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center p-8 bg-muted/50 rounded-xl">
                        <div>
                        ...
                        <div className="flex items-center space-x-4">
                            <button
                                ...
                                className="w-8 h-8 rounded-full bg-muted hover:bg-muted flex items-center justify-center transition-colors"
                            >
        ...
        if (!subSections?.length) {
            return (
                <div className="text-center py-12">
                    <p className="text-muted-foreground">{t('No content available')}</p>
                </div>
            );
        }

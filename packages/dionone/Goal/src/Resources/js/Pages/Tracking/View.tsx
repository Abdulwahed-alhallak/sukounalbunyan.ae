...
export default function View() {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'ahead': return 'bg-muted text-foreground';
            case 'on_track': return 'bg-muted text-foreground';
            case 'behind': return 'bg-muted text-foreground';
            case 'critical': return 'bg-muted text-destructive';
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="text-center p-4 bg-muted/50 rounded-lg">
                                <DollarSign className="h-8 w-8 text-foreground mx-auto mb-2" />
                                <p className="text-sm text-muted-foreground">{t('Target Amount')}</p>
                                <p className="text-2xl font-bold text-foreground">{formatCurrency(tracking.goal?.target_amount || 0)}</p>
                            </div>
                            <div className="text-center p-4 bg-muted/50 rounded-lg">
                                <Target className="h-8 w-8 text-foreground mx-auto mb-2" />
                                <p className="text-sm text-muted-foreground">{t('Current Amount')}</p>
                                <p className="text-2xl font-bold text-foreground">{formatCurrency(tracking.current_amount)}</p>
                            </div>
                            <div className="text-center p-4 bg-muted/50 rounded-lg">
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="flex justify-between items-center py-2 border-b">
                                ...
                                <span className="font-medium text-foreground">{formatCurrency(tracking.contribution_amount)}</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b">
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="flex justify-between items-center py-2 border-b">
                                ...
                                <span className={`font-medium ${tracking.days_remaining < 0 ? 'text-destructive' : tracking.days_remaining < 30 ? 'text-muted-foreground' : 'text-foreground'}`}>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <p className="text-sm text-muted-foreground mb-1">{t('Goal Name')}</p>
                                    <p className="font-medium">{tracking.goal.goal_name}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-muted-foreground mb-1">{t('Goal Type')}</p>
                                    <p className="font-medium capitalize">{tracking.goal.goal_type?.replace('_', ' ')}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-muted-foreground mb-1">{t('Start Date')}</p>
                                    <p className="font-medium">{formatDate(tracking.goal.start_date)}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-muted-foreground mb-1">{t('Target Date')}</p>

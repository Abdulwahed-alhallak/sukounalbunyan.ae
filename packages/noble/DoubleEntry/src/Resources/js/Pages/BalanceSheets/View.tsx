                            ...
                            return (
                                <React.Fragment key={subSection}>
                                    <TableRow key={`${subSection}-header`}>
                                        <TableCell colSpan={3} className="font-semibold text-foreground capitalize">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-2 ml-2">
                                    <span className={`px-2 py-1 rounded-full text-sm ${
                                        balanceSheet.is_balanced ? 'bg-muted text-foreground' : 'bg-muted text-destructive'
                                    }`}>
                                        {t(balanceSheet.is_balanced ? 'Balanced' : 'Unbalanced')}
                                    </span>
                                    <span className={`px-2 py-1 rounded-full text-sm ${
                                        balanceSheet.status === 'finalized' ? 'bg-muted text-foreground' : 'bg-muted text-foreground'
                                    }`}>
                    ...
                    <CardContent>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="text-center p-6 bg-gradient-to-br from-muted/50 to-muted rounded-xl border border-border shadow-sm hover:shadow-md transition-shadow">
                                <h4 className="font-semibold text-foreground mb-2">{t('Total Assets')}</h4>
                                <p className="text-3xl font-bold text-foreground tabular-nums">
                                    {formatCurrency(balanceSheet.total_assets)}
                                </p>
                            </div>
                            <div className="text-center p-6 bg-gradient-to-br from-muted/50 to-muted rounded-xl border border-border shadow-sm hover:shadow-md transition-shadow">
                                <h4 className="font-semibold text-destructive mb-2">{t('Total Liabilities')}</h4>
                                <p className="text-3xl font-bold text-destructive tabular-nums">
                                    {formatCurrency(balanceSheet.total_liabilities)}
                                </p>
                            </div>
                            <div className="text-center p-6 bg-gradient-to-br from-muted/50 to-muted rounded-xl border border-border shadow-sm hover:shadow-md transition-shadow">
                                <h4 className="font-semibold text-foreground mb-2">{t('Total Equity')}</h4>
                                <p className="text-3xl font-bold text-foreground tabular-nums">
                    ...
                    <CardContent className="p-8">
                        ...
                        {viewType === 'vertical' ? (
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                                ...
                                <div>
                                    <h3 className="text-xl font-bold text-foreground mb-6">{t('Liabilities & Equity')}</h3>

                                    {/* Equity */}
                                    {groupedItems.equity && (
                                        <div className="mb-6">
                                            <h4 className="font-semibold text-foreground mb-3">{t('Equity')}</h4>
                                            {Object.entries(groupedItems.equity)?.map(([subSection, items]) => (
                                                <div key={subSection} className="mb-4">
                                                    {items?.map((item) => (
                                                        <div key={item.id} className="flex justify-between py-1 text-sm">
                                                            <span className="text-foreground">{item.account?.account_name}</span>
                                    ...
                                    {groupedItems.liabilities && (
                                        <div className="mb-6">
                                            <h4 className="font-semibold text-foreground mb-3">{t('Liabilities')}</h4>
                                            {Object.entries(groupedItems.liabilities)?.map(([subSection, items]) => {
                                                ...
                                                return (
                                                    <div key={subSection} className="mb-4">
                                                        <h5 className="font-medium text-muted-foreground mb-2 capitalize">{subSection.replace('_', ' ')}</h5>
                                                        {items?.map((item) => (
                                                            <div key={item.id} className="flex justify-between items-center py-1 text-sm ml-4">
                                                                <div className="flex justify-between w-full">
                                                                    <span className="text-foreground">{item.account?.account_name}</span>
                                                                    <div className="flex gap-8">
                                                                        <span className="text-muted-foreground">{item.account?.account_code}</span>
                                                                        <span className="text-foreground tabular-nums">{formatCurrency(item.amount)}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                ...
                                {/* Right Side - Assets */}
                                <div>
                                    <h3 className="text-xl font-bold text-foreground mb-6">{t('Assets')}</h3>

                                    {groupedItems.assets && (
                                        <div>
                                            <h4 className="font-semibold text-foreground mb-3">{t('Assets')}</h4>
                                        ...
                                        <div key={note.id} className="p-4 bg-muted/50 rounded-lg">
                                            <div className="flex justify-between items-start">
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-foreground mt-2">{note.note_content}</p>
                                                </div>

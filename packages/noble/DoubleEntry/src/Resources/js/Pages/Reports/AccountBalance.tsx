...
export default function AccountBalance({ financialYear }: AccountBalanceProps) {
    ...
    const handleDownloadPDF = () => {
        const printUrl =
            route('double-entry.reports.account-balance.print') +
    ...
    const typeColors: Record<string, string> = {
        Assets: 'bg-muted/50',
        Liabilities: 'bg-muted/50',
        Equity: 'bg-muted/50',
        Revenue: 'bg-muted/50',
        Expenses: 'bg-muted/50',
    };
        ...
        <Card className="shadow-sm">
            <CardContent className="bg-muted/50/50 border-b p-6">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                    ...
                    <div>
                        <label className="mb-2 block text-sm font-medium text-foreground">{t('Account Type')}</label>
                        ...
                        <div className="min-w-[900px]">
                            {data.as_of_date && (
                                <div className="border-b bg-muted/50 p-4">
                                    <h3 className="text-lg font-semibold">{t('Account Balance Summary')}</h3>
                                ...
                                <tbody>
                                    {Object.entries(data.grouped)?.map(([type, group]) => (
                                        <>
                                            <tr
                                                key={`header-${type}`}
                                                className={`${typeColors[type as keyof typeof typeColors] || 'bg-muted/50'} font-semibold`}
                                            >
                                            ...
                                            {group.accounts?.map((account, idx) => (
                                                <tr key={`${type}-${idx}`} className="border-t hover:bg-muted/50">
                                            ...
                                            <tr key={`subtotal-${type}`} className="border-t-2 bg-muted font-semibold">

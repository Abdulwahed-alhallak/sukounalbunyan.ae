...
namespace Noble\Timesheet\Http\Controllers;
...
use Noble\Timesheet\Http\Requests\StoreTimesheetRequest;
use Noble\Timesheet\Http\Requests\UpdateTimesheetRequest;
use Noble\Timesheet\Events\CreateTimesheet;
use Noble\Timesheet\Events\UpdateTimesheet;
use Noble\Timesheet\Events\DestroyTimesheet;
use App\Models\User;
...
class TimesheetController extends Controller
{
        ...
        if (Auth::user()->can('manage-timesheet')) {
            ...
            if (module_is_active('Taskly')) {
                $timesheets->getCollection()->transform(function ($timesheet) {
                    try {
                        if ($timesheet->project_id && class_exists('\Noble\Taskly\Models\Project')) {
                            $project = \Noble\Taskly\Models\Project::find($timesheet->project_id);
                            $timesheet->project_name = $project ? $project->name : 'N/A';
                        }
                        if ($timesheet->task_id && class_exists('\Noble\Taskly\Models\ProjectTask')) {
                            $task = \Noble\Taskly\Models\ProjectTask::find($timesheet->task_id);
            ...
            $projects = [];
            if (module_is_active('Taskly') && class_exists('\Noble\Taskly\Models\Project')) {
                try {
                    if (Auth::user()->can('manage-any-timesheet')) {
                        $projects = \Noble\Taskly\Models\Project::where('created_by', creatorId())
                            ->select('id', 'name')
                            ->get();
                    } else {
                        $projects = \Noble\Taskly\Models\Project::whereHas('users', fn($q) => $q->where('user_id', Auth::id()))
                            ->select('id', 'name')
                            ->get();
                    }
                } catch (\Exception $e) {
    ...
    public function getAttendanceHours($userId, $date)
    {
        ...
        $attendance = \Noble\Hrm\Models\Attendance::where('employee_id', $userId)
            ->whereDate('date', $date)
            ->first();

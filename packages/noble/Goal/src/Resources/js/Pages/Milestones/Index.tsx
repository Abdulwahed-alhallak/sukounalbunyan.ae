...
export default function Index() {
    ...
    const handleFilter = () => {
        ...
        router.get(
            route('goal.milestones.index'),
            { ...filterParams, per_page: perPage, sort: sortField, direction: sortDirection },
            {
                preserveState: true,
                replace: true,
            }
    ...
    const tableColumns = [
        ...
        ...(auth.user?.permissions?.some((p: string) =>
            ['view-goal-milestones', 'edit-goal-milestones', 'delete-goal-milestones'].includes(p)
        )
            ? [
                  {
                      key: 'actions',
                                      ...
                                      <Tooltip delayDuration={0}>
                                          <TooltipTrigger asChild>
                                              <Button
                                                  variant="ghost"
                                                  size="sm"
                                                  onClick={() => setViewingItem(milestone)}
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[{ label: t('Goal') }, { label: t('Milestones') }]}
        ...
        >
            ...
            <Card className="shadow-sm">
                <CardContent className="bg-muted/50/50 border-b p-6">
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <PerPageSelector routeName="goal.milestones.index" filters={{ ...filters }} />
                            <div className="relative">
                                <FilterButton showFilters={showFilters} onToggle={() => setShowFilters(!showFilters)} />
                                {(() => {
                                    const activeFilters = [filters.goal_id, filters.status, filters.date_range].filter(
                ...
                <CardContent className="bg-muted/50/30 border-t px-4 py-2">
                    <Pagination
                        data={milestones || { current_page: 1, last_page: 1, per_page: 10, total: 0, from: 0, to: 0 }}
            ...
            <Dialog open={modalState.isOpen} onOpenChange={closeModal}>
                ...
                {modalState.mode === 'edit' && modalState.data && (
                    <EditMilestone milestone={modalState.data} goals={goals} onSuccess={closeModal} />
                )}
            </Dialog>

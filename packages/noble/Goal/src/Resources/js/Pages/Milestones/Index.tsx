        ...
        },
        {
            ...
            render: (value: string) => (
                <span className={`px-2 py-1 rounded-full text-sm ${
                    value === 'achieved' ? 'bg-muted text-foreground' :
                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('view-goal-milestones') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => setViewingItem(milestone)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('edit-goal-milestones') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', milestone)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                ...
                {showFilters && (
                    <CardContent className="p-6 bg-muted/50/30 border-b">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Target Date Range')}</label>
                                <DateRangePicker

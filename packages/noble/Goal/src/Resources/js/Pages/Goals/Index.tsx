...
export default function Index() {
    ...
    const tableColumns = [
            ...
            render: (_: any, goal: Goal) => {
                ...
                return (
                    <div className="flex items-center gap-2 min-w-24">
                        ...
                        <span className="text-xs font-medium text-muted-foreground min-w-8">{progress}%</span>
                    </div>
                );
            }
        },
        ...
        },
        {
            ...
            render: (value: string) => (
                <span className={`px-2 py-1 rounded-full text-sm ${
                    value === 'critical' ? 'bg-muted text-destructive' :
                    value === 'high' ? 'bg-muted text-foreground' :
        ...
        },
        {
            ...
            render: (value: string) => (
                <span className={`px-2 py-1 rounded-full text-sm ${
                    value === 'completed' ? 'bg-muted text-foreground' :
                    value === 'active' ? 'bg-muted text-foreground' :
                    'bg-muted text-foreground'
                }`}>
                    {t(value.charAt(0).toUpperCase() + value.slice(1))}
                </span>
                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {goal.status === 'draft' && auth.user?.permissions?.includes('active-goals') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.post(route('goal.goals.active', goal.id))} className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('view-goals') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => setViewingItem(goal)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {goal.status === 'draft' && auth.user?.permissions?.includes('edit-goals') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', goal)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        <div className="overflow-auto max-h-[70vh] p-4">
                            {goals?.data && goals.data.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                    {goals.data?.map((goal) => (
                                        <Card key={goal.id} className="border border-border hover:shadow-md transition-shadow">
                                            <div className="p-4">
                                                <div className="flex items-center justify-between mb-3">
                                                    <h3 className="font-semibold text-base text-foreground truncate">{goal.goal_name}</h3>
                                                    <span className={`px-2 py-1 rounded-full text-sm ${
                                                        goal.status === 'completed' ? 'bg-muted text-foreground' :
                                                        goal.status === 'active' ? 'bg-muted text-foreground' :
                                                        'bg-muted text-foreground'
                                                    }`}>
                                                ...
                                                <div className="space-y-3 mb-4">
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div>
                                                            ...
                                                            <p className="text-xs text-foreground">{t(goal.goal_type.replace('_', ' '))}</p>
                                                        </div>
                                                        <div className="text-end">
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Category')}</p>
                                                            <p className="text-xs text-foreground truncate">{goal.category?.category_name || '-'}</p>
                                                        </div>
                                                    </div>
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div>
                                                            ...
                                                            <span className={`px-2 py-1 rounded-full text-xs ${
                                                                goal.priority === 'critical' ? 'bg-muted text-destructive' :
                                                                goal.priority === 'high' ? 'bg-muted text-foreground' :
                                                                goal.priority === 'medium' ? 'bg-muted text-foreground' :
                                                                'bg-muted text-foreground'
                                                            }`}>
                                                        ...
                                                        <div className="text-end">
                                                            <p className="text-xs font-medium text-muted-foreground mb-1">{t('Target Date')}</p>
                                                            <p className="text-xs text-foreground">{formatDate(goal.target_date)}</p>
                                                        </div>
                                                    </div>
                                                    <div className="bg-muted/50 rounded-lg p-3">
                                                        <div className="flex justify-between items-center mb-2">
                                                            <span className="text-sm font-semibold text-foreground">{t('Target')}</span>
                                                            <span className="text-lg font-bold text-foreground">{formatCurrency(goal.target_amount)}</span>
                                                        </div>
                                                        <div className="flex justify-between items-center mb-3">
                                                            <span className="text-sm text-muted-foreground">{t('Current')}</span>
                                                            <span className="text-sm font-semibold text-foreground">{formatCurrency(goal.current_amount)}</span>
                                                        </div>

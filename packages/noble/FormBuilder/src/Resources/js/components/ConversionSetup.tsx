    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (data.is_active && selectedModule && Object.keys(moduleFields).length > 0) {
            ...
            if (unmappedFields.length > 0) {
                const fieldNames = unmappedFields
                    ?.map((key) =>
                ...
                toast.error(
                    t('Please map all required fields: {{fields}}', {
                        fields: fieldNames,
                    })
                );
                return;
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <Label htmlFor="module" className="mb-2 block text-sm font-medium">
                                    {t('Select Module')}
                                </Label>
                            ...
                            {getAvailableSubmodules().length > 0 && (
                                <div>
                                    <Label htmlFor="submodule" className="mb-2 block text-sm font-medium">
                                        {t('Select Submodule')}
                                    </Label>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <Badge variant="secondary">
                                    {Object.keys(data.field_mappings).length} / {Object.keys(moduleFields).length}{' '}
                                    {t('mapped')}
                                </Badge>
                            </div>
                            ...
                            <div className="space-y-3">
                                {Object.entries(moduleFields)?.map(([fieldKey, fieldConfig]) => (
                                    <div key={fieldKey} className="flex items-center gap-4 rounded-lg border p-3">
                                        ...
                                        <div className="flex-1">
                                            {fieldConfig?.multiple ? (
                                                <MultiSelect
                                                    options={
                                                        ...
                                                        initialData?.form_fields
                                                            .filter((field) => {
                                                                const moduleFieldType =
                                                                    typeof fieldConfig === 'object' &&
                                                                    fieldConfig?.type;
                                                                return (
                                                                    !moduleFieldType || field.type === moduleFieldType
                                                                );
                                                            })
                                                            ?.map((field) => ({
                                                ...
                                                <Select
                                                    value={
                                                        data.field_mappings[fieldKey]
                                                            ? String(data.field_mappings[fieldKey])
                                                            : 'not_mapped'
                                                    }
                                            ...
                                            {data.field_mappings[fieldKey] &&
                                                (Array.isArray(data.field_mappings[fieldKey])

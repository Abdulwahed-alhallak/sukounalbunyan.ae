...
namespace Noble\Pos\Http\Controllers;
...
use Noble\Pos\Models\PosItem;
use Noble\Pos\Http\Requests\StorePosRequest;
use Noble\Pos\Events\CreatePosSale;
use Noble\Pos\Models\PosPayment;
        ...
        $products = $query->get()
            ->map(function($product) {
                ...
                if ($product->tax_ids && is_array($product->tax_ids) && !empty($product->tax_ids)) {
                    $taxes = \Noble\ProductService\Models\ProductServiceTax::whereIn('id', $product->tax_ids)
                        ->where('created_by', creatorId())

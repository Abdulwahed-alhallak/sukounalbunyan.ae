                                    ...
                                    {purchasedProducts.slice(0, 10)?.map((product, index) => (
                                        <div key={index} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
                                            ...
                                            <div className="text-right">
                                                <p className="text-sm font-bold text-foreground">{formatCurrency(product.total_spent)}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            {recentPurchases && recentPurchases.length > 0 ? (
                                <div className="space-y-3 max-h-80 overflow-y-auto">
                                    {recentPurchases.slice(0, 5)?.map((purchase) => (
                                        <div key={purchase.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
                                            <div className="flex-1">
                                                <h4 className="font-medium text-sm text-foreground">{purchase.sale_number}</h4>
                                                <p className="text-xs text-muted-foreground mt-1">{purchase.warehouse?.name || '-'}</p>
                                            </div>
                                            <div className="text-right">

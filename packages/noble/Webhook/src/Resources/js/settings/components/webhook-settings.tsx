...
const WebhookSettings: React.FC<WebhookSettingsProps> = ({
    ...
    const handleToggle = async (webhookId: number) => {
        router.put(
            route('webhook.settings.toggle', webhookId),
            {},
            {
                preserveScroll: true,
                onSuccess: () => {
        ...
        <Card>
            ...
            <CardContent>
                <div className="space-y-6">
                    {/* Webhook List */}
                    <div>
                        <h3 className="mb-4 text-base font-medium">{t('Configured Webhooks')}</h3>
                        {webhooks.length === 0 ? (
                            ...
                            <div className="max-h-[400px] space-y-3 overflow-y-auto pe-2">
                                {webhooks?.map((webhook) => (
                                    <div
                                        key={webhook.id}
                                        className={`flex items-center justify-between rounded-lg border p-4 ${webhook.is_active ? 'bg-muted/50/50' : 'bg-muted/50 opacity-75'}`}
                                    >
                                        <div className="flex-1">
                                            <div className="mb-2 flex items-center gap-2">
                                                <Badge variant="outline">{webhook.method}</Badge>
                                                <span className="font-medium">
                                                    {webhook.webhook_module?.submodule || 'Unknown Event'}
                                                </span>
                                                <Badge variant="secondary">
                                                    {getPackageAlias(
                                                        webhook.webhook_module?.module || 'Unknown',
                                                        props
                                                    )}
                                                </Badge>
                                                <Badge

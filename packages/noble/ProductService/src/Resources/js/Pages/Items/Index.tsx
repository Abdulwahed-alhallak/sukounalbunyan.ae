...
export default function Index() {
    ...
    const dropboxBtn = usePageButtons('dropboxBtn', {
        module: 'Product & Service Products',
        settingKey: 'Dropbox Product & Service Products',
    });
    ...
    const handleFilter = () => {
        router.get(
            route('product-service.items.index'),
    ...
    const tableColumns = [
                ...
                return isImage ? (
                    <div className="relative h-12 w-12">
                        <img
                            src={imageUrl}
                            alt="Image"
                            className="h-12 w-12 cursor-pointer rounded-md border object-cover transition-transform hover:scale-110"
                        ...
                        />
                        <div className="flex hidden h-12 w-12 items-center justify-center rounded-md border bg-muted">
                            <Image className="h-6 w-6 text-muted-foreground" />
                        </div>
                    </div>
                ) : (
                    <div
                        className="flex h-12 w-12 cursor-pointer items-center justify-center rounded-md border bg-muted transition-colors hover:bg-muted"
                        onClick={() => {
        ...
        },
        {
            ...
            sortable: true,
            render: (value: string) => (
                <span className="rounded-full bg-muted px-2 py-1 text-sm capitalize text-foreground">{t(value)}</span>
            ),
        },
        ...(auth.user?.permissions?.some((p: string) =>
            ['view-product-service-item', 'edit-product-service-item', 'delete-product-service-item'].includes(p)
        )
            ? [
                  {
                      key: 'actions',
                                      ...
                                      <Tooltip delayDuration={0}>
                                          <TooltipTrigger asChild>
                                              <Button
                                                  ...
                                                  size="sm"
                                                  onClick={() =>
                                                      router.visit(route('product-service.items.show', item.id))
                                                  }
                                      ...
                                      <Tooltip delayDuration={0}>
                                          <TooltipTrigger asChild>
                                              <Button
                                                  ...
                                                  size="sm"
                                                  onClick={() =>
                                                      router.visit(route('product-service.items.edit', item.id))
                                                  }
                                                  className="h-8 w-8 p-0 text-foreground hover:text-foreground"
                                              >
    ...
    return (
        <TooltipProvider>
            <AuthenticatedLayout
                breadcrumbs={[{ label: t('Product & Service') }, { label: t('Items') }]}
                            ...
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="outline"
                                        size="sm"
                                        onClick={() => router.visit(route('product-service.stock.index'))}
                                    >
                        ...
                        {auth.user?.permissions?.includes('create-product-service-item') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button
                                        size="sm"
                                        onClick={() => router.visit(route('product-service.items.create'))}
                                    >
                        ...
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="relative">
                                    ...
                                    />
                                    {(() => {
                                        const activeFilters = [filters.type, filters.category_id].filter(
                                            Boolean
                                        ).length;
                                        return (
                                            activeFilters > 0 && (
                                    ...
                                    <DataTable
                                        ...
                                        emptyState={
                                            <NoRecordsFound
                                                ...
                                                onCreateClick={() =>
                                                    router.visit(route('product-service.items.create'))
                                                }
                            ...
                            <div className="max-h-[70vh] overflow-auto p-6">
                                {items.data.length > 0 ? (
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                                        {items.data?.map((item) => (
                                            <Card key={item.id} className="flex h-full flex-col border border-border">
                                                        ...
                                                        {item.image ? (
                                                            <div className="relative h-12 w-12">
                                                                <img
                                                                    ...
                                                                    onClick={() =>
                                                                        window.open(getImagePath(item.image), '_blank')
                                                                    }
                                                                    onError={(e) => {
                                                                        const target = e.target as HTMLImageElement;
                                                                        target.style.display = 'none';

...
export default function Index() {
    ...
    const tableColumns = [
            ...
            render: (value: string) => {
                ...
                return isImage ? (
                    <div className="relative w-12 h-12">
                        ...
                        />
                        <div className="hidden w-12 h-12 bg-muted rounded-md border flex items-center justify-center">
                            <Image className="w-6 h-6 text-muted-foreground" />
                        </div>
                    </div>
                ) : (
                    <div className="w-12 h-12 bg-muted rounded-md border flex items-center justify-center cursor-pointer hover:bg-muted transition-colors" onClick={() => {
        ...
        },
        {
            ...
            sortable: true,
            render: (value: string) => (
                <span className="px-2 py-1 rounded-full text-sm bg-muted text-foreground capitalize">
                    {t(value)}
                </span>
            )
        },
                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('view-product-service-item') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.visit(route('product-service.items.show', item.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('edit-product-service-item') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.visit(route('product-service.items.edit', item.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                ...
                {showFilters && (
                    <CardContent className="p-6 bg-muted/50/30 border-b">
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Item Type')}</label>
                        ...
                        <div className="overflow-auto max-h-[70vh] p-6">
                            {items.data.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                    {items.data?.map((item) => (
                                        <Card key={item.id} className="border border-border h-full flex flex-col">
                                            <div className="p-4 flex flex-col flex-1">
                                                ...
                                                <div className="space-y-2 mb-3 flex-1">
                                                    ...
                                                    {item.sale_price && (
                                                        <div className="flex justify-between">
                                                            <span className="text-sm text-muted-foreground">{t('Sale Price')}</span>
                                                ...
                                                <TooltipProvider>
                                                    {auth.user?.permissions?.includes('view-product-service-item') && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => router.visit(route('product-service.items.show', item.id))} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                    <Eye className="h-4 w-4" />
                                                                </Button>
                                                            </TooltipTrigger>
                                                    ...
                                                    {auth.user?.permissions?.includes('edit-product-service-item') && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => router.visit(route('product-service.items.edit', item.id))} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                    <Edit className="h-4 w-4" />
                                                                </Button>
                                                            </TooltipTrigger>

...
export default function Index() {
    ...
    const handleSort = (field: string) => {
        ...
        router.get(
            route('budget-planner.budget-periods.index'),
            ...
            const hasAnyActionPermission = (budgetperiod: BudgetPeriod) => {
                ...
                return (
                    (budgetperiod.status === 'draft' &&
                        (permissions.includes('approve-budget-periods') ||
                            permissions.includes('edit-budget-periods') ||
                            permissions.includes('delete-budget-periods'))) ||
                          ...
                          render: (_: any, budgetperiod: BudgetPeriod) => {
                              ...
                              return (
                                  <div className="flex gap-1">
                                      <TooltipProvider>
                                          {budgetperiod.status === 'draft' &&
                                              auth.user?.permissions?.includes('approve-budget-periods') && (
                                                      ...
                                                      <TooltipTrigger asChild>
                                                          <Button
                                                              ...
                                                              onClick={() =>
                                                                  router.post(
                                                                      route(
                                                                          'budget-planner.budget-periods.approve',
                                                                          budgetperiod.id
                                                                      )
                                                                  )
                                                              }
                                          ...
                                          {budgetperiod.status === 'approved' &&
                                              auth.user?.permissions?.includes('active-budget-periods') && (
                                                  <Tooltip delayDuration={0}>
                                                      <TooltipTrigger asChild>
                                                          <Button
                                                              variant="ghost"
                                                              ...
                                                              onClick={() =>
                                                                  router.post(
                                                                      route(
                                                                          'budget-planner.budget-periods.active',
                                                                          budgetperiod.id
                                                                      )
                                                                  )
                                                              }
                                          ...
                                          {budgetperiod.status === 'active' &&
                                              auth.user?.permissions?.includes('close-budget-periods') && (
                                                  <Tooltip delayDuration={0}>
                                                      <TooltipTrigger asChild>
                                                          <Button
                                                              variant="ghost"
                                                              ...
                                                              onClick={() =>
                                                                  router.post(
                                                                      route(
                                                                          'budget-planner.budget-periods.close',
                                                                          budgetperiod.id
                                                                      )
                                                                  )
                                                              }
                                          ...
                                          {budgetperiod.status === 'draft' &&
                                              auth.user?.permissions?.includes('edit-budget-periods') && (
                                                  <Tooltip delayDuration={0}>
                                                      <TooltipTrigger asChild>
                                                          <Button
                                                              variant="ghost"
                                                              size="sm"
                                                              onClick={() => openModal('edit', budgetperiod)}
                                                              className="h-8 w-8 p-0 text-foreground hover:text-foreground"
                                                          >
                                          ...
                                          {budgetperiod.status === 'draft' &&
                                              auth.user?.permissions?.includes('delete-budget-periods') && (
                                                  <Tooltip delayDuration={0}>
                                                      <TooltipTrigger asChild>
                                                          <Button
                                                              variant="ghost"
        ...
        >
            ...
            <Card className="shadow-sm">
                ...
                <CardContent className="bg-muted/50/50 border-b p-6">
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <PerPageSelector routeName="budget-planner.budget-periods.index" filters={{ ...filters }} />
                ...
                <CardContent className="bg-muted/50/30 border-t px-4 py-2">
                    <Pagination
                        data={budgetperiods || { current_page: 1, last_page: 1, per_page: 10, total: 0, from: 0, to: 0 }}
            ...
            <Dialog open={modalState.isOpen} onOpenChange={closeModal}>
                ...
                {modalState.mode === 'edit' && modalState.data && (
                    <Edit budgetperiod={modalState.data} onSuccess={closeModal} />
                )}
            </Dialog>

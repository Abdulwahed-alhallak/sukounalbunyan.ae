...
export default function Index() {
    ...
    const {
        ...
        auth,
    } = usePage<CreditNoteIndexProps>().props;
    ...
    const handleFilter = () => {
        router.get(
            route('account.credit-notes.index'),
    ...
    const tableColumns = [
        ...
        ...(auth.user?.permissions?.some((p: string) =>
            ['view-credit-notes', 'approve-credit-notes', 'delete-credit-notes'].includes(p)
        )
            ? [
                  {
                      key: 'actions',
                      ...
                      render: (_: any, creditNote: CreditNote) => (
                          <div className="flex gap-1">
                              <TooltipProvider>
                                      ...
                                      <Tooltip delayDuration={0}>
                                          <TooltipTrigger asChild>
                                              <Button
                                                  ...
                                                  size="sm"
                                                  onClick={() =>
                                                      router.get(route('account.credit-notes.show', creditNote.id))
                                                  }
                                                  className="h-8 w-8 p-0 text-foreground hover:text-foreground"
                                              >
                                  ...
                                  {creditNote.status === 'draft' &&
                                      auth.user?.permissions?.includes('delete-credit-notes') && (
                                          <Tooltip delayDuration={0}>
                                              <TooltipTrigger asChild>
                                                  <Button
                                                      variant="ghost"
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[{ label: t('Accounting'), url: route('account.index') }, { label: t('Credit Notes') }]}
                    ...
                    <CardContent className="bg-muted/50/30 border-b p-6">
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                            ...
                            <div>
                                ...
                                <Select
                                    value={filters.sales_return_id}
                                    onValueChange={(value) => setFilters({ ...filters, sales_return_id: value })}
                                >
                                    <SelectTrigger>
                                ...
                                <DataTable
                                    ...
                                    emptyState={
                                        <NoRecordsFound
                                            ...
                                            description={t(
                                                'Credit notes are automatically created from sales returns.'
                                            )}
                                            hasFilters={
                                                !!(
                                                    filters.search ||
                            ...
                            {creditNotes.data.length > 0 ? (
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                                    {creditNotes.data?.map((creditNote) => (
                                        <Card key={creditNote.id} className="flex flex-col border border-border">
                                                ...
                                                <div className="mb-3">
                                                    {auth.user?.permissions?.includes('view-credit-notes') ? (
                                                        <h3
                                                            ...
                                                            onClick={() =>
                                                                router.get(
                                                                    route('account.credit-notes.show', creditNote.id)
                                                                )
                                                            }
                                                        >
                                                        ...
                                                        <div className="grid grid-cols-2 gap-2 text-xs">
                                                            <div className="flex justify-between">
                                                                ...
                                                                <span className="font-medium">
                                                                    {formatCurrency(
                                                                        parseFloat(creditNote.total_amount.toString())
                                                                    )}
                                                                </span>
                                                            </div>
                                                            <div className="flex justify-between">
                                                                ...
                                                                <span className="font-medium">
                                                                    {formatCurrency(
                                                                        parseFloat(creditNote.applied_amount.toString())
                                                                    )}
                                                                </span>
                                                            </div>
                                                        </div>
                                                        <div className="mt-2 border-t pt-2">
                                                            <div className="flex items-center justify-between">
                                                                ...
                                                                <span className="text-lg font-bold text-foreground">
                                                                    {formatCurrency(
                                                                        parseFloat(creditNote.balance_amount.toString())
                                                                    )}
                                                                </span>
                                                            </div>
                                                        </div>
                                                                ...
                                                                <Tooltip delayDuration={0}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                            ...
                                                                            size="sm"
                                                                            onClick={() =>
                                                                                openDeleteDialog(creditNote.id)
                                                                            }
                                                                            className="h-8 w-8 p-0 text-destructive hover:text-destructive"
                                                                        >
                                                                            <Trash2 className="h-4 w-4" />

...
class DashboardController extends Controller
{
    ...
    private function vendorDashboard()
    {
        ...
        if (class_exists('\\Noble\\Account\\Models\\DebitNote')) {
            $recentDebitNotes = \Noble\Account\Models\DebitNote::where('vendor_id', $user->id)
                ->latest()
                ->limit(5)
                ->get()
                ->map(function($note) {
    ...
    private function clientDashboard()
    {
        ...
        if (class_exists('\\Noble\\Account\\Models\\CreditNote')) {
            $recentCreditNotes = \Noble\Account\Models\CreditNote::where('customer_id', $user->id)
                ->latest()
                ->limit(5)
                ->get()
                ->map(function($note) {

...
use Noble\Taskly\Events\DestroyProject;
use Noble\Taskly\Events\DestroyProjectMilestone;
use Noble\Taskly\Events\ProjectDeleteClient;
use Noble\Taskly\Events\ProjectDeleteShareToClient;
use Noble\Taskly\Events\ProjectInviteMember;
...
use Noble\Taskly\Http\Requests\StoreProjectRequest;
use Noble\Taskly\Http\Requests\UpdateProjectRequest;
use Noble\Taskly\Models\ActivityLog;
...
class ProjectController extends Controller
{
    ...
    public function store(StoreProjectRequest $request)
    {
        if (Auth::user()->can('create-project')) {
            ...
            // Ecosystem Auto-Geolocator Logic
            if (!empty($validated['latitude']) && !empty($validated['longitude'])) {
                $locationId = DB::table('job_locations')->insertGetId([
                    'name' => $validated['location_name'] ?: $validated['name'],
                    'latitude' => $validated['latitude'],
                    'longitude' => $validated['longitude'],
                    'geofence_radius_meters' => $validated['geofence_radius'] ?? 100,
                    'created_by' => creatorId(),
                ]);
                $project->job_location_id = $locationId;
            }
    ...
    public function show(Project $project)
    {
        if (Auth::user()->can('view-project') && $project->created_by == creatorId()) {
            ...
            $taskStages = \Noble\Taskly\Models\TaskStage::where('created_by', creatorId())->orderBy('order')->get();
            $chartData = [];
            for ($i = 4; $i >= 0; $i--) {
    ...
    public function update(UpdateProjectRequest $request, Project $project)
    {
        if (Auth::user()->can('edit-project')) {
            ...
            // Ecosystem Auto-Geolocator Logic (Update mode)
            if (!empty($validated['latitude']) && !empty($validated['longitude'])) {
                if ($project->job_location_id) {
                    DB::table('job_locations')->where('id', $project->job_location_id)->update([
                        'name' => $validated['location_name'] ?: $validated['name'],
                        'latitude' => $validated['latitude'],
                        'longitude' => $validated['longitude'],
                        'geofence_radius_meters' => $validated['geofence_radius'] ?? 100,

...
use Noble\Taskly\Events\DestroyProjectTask;
use Noble\Taskly\Events\UpdateProjectTask;
...
use Noble\Taskly\Http\Requests\StoreProjectTaskRequest;
use Noble\Taskly\Http\Requests\UpdateProjectTaskRequest;
use Noble\Taskly\Models\ActivityLog;
use Noble\Taskly\Models\TaskComment;
use Noble\Taskly\Models\TaskSubtask;
use Noble\Taskly\Models\TaskAttachment;
use Noble\Taskly\Events\CreateTaskSubtask;
use Noble\Taskly\Events\DestroyTaskComment;
...
class ProjectTaskController extends Controller
{
    ...
    public function store(StoreProjectTaskRequest $request)
    {
        if (Auth::user()->can('create-project-task')) {
            ...
            if ($request->has('media_paths')) {
                $task->syncAttachments($request->input('media_paths'));
            }
    ...
    public function destroyAttachment(TaskAttachment $attachment)
    {
        if (Auth::user()->can('delete-project-task')) {
            $attachment->delete();
            return response()->json(['success' => true, 'message' => __('The attachment has been deleted successfully.')]);
        }
        return response()->json(['error' => __('Permission denied')], 403);
    }
}
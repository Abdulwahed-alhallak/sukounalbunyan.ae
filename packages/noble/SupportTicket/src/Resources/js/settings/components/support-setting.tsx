...
export default function SupportSettings({ userSettings, fields: initialFields, auth }: SupportSettingsProps) {
    ...
    const saveSettings = () => {
        ...
        router.post(
            route('support-ticket.settings.update'),
            {
                settings: {
                    ...settings,
                    fields: fields,
                                ...
                                <TooltipProvider>
                                    <Tooltip delayDuration={0}>
                                        <TooltipTrigger asChild>
                                            <Button onClick={addField} size="sm" className="text-background">
                            ...
                            <div>
                                {fields?.map((field, index) => (
                                    <div
                                        ...
                                        className={`border-b bg-card px-4 py-3 ${canEdit ? 'cursor-move' : ''} hover:bg-muted/50`}
                                    >
                                        <div className="grid grid-cols-12 items-center gap-4">
                                            <div className="col-span-1">
                                                {canEdit && <GripVertical className="h-4 w-4 text-muted-foreground" />}
                                            </div>
                                                ...
                                                >
                                                    ...
                                                    <SelectContent>
                                                        {Object.entries(fieldTypes)?.map(([key, value]) => {
                                                            if (
                                                                (key === 'file' || key === 'select') &&
                                                                (!field.custom_id || field.custom_id > 6)
                                                            ) {
                                                                return null;
                                                            }
                                                            return (
                                            ...
                                            <div className="col-span-2">
                                                <Select
                                                    ...
                                                    onValueChange={(value) =>
                                                        updateField(index, { is_required: value === '1' })
                                                    }
                                                    disabled={!canEdit}
                                                >
                                                    <SelectTrigger>

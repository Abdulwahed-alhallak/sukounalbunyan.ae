...
export default function Index() {
        ...
        },
        {
            ...
            render: (value: number, contracttype: ContractType) => {
                ...
                if (totalCount === 0) {
                    return (
                        <div className="flex">
                            <span className="text-sm font-medium text-muted-foreground">{t('No contracts')}</span>
                        </div>
                    );
                }
                        ...
                        <div className="flex flex-wrap gap-1">
                            {displayContracts?.map((contract) => (
                                <Badge
                                    key={contract.id}
                                    variant="outline"
                                    className="border-border bg-muted/50 text-xs text-foreground hover:bg-muted"
                                >
                          ...
                          <div className="flex gap-1">
                              <TooltipProvider>
                                  {auth.user?.permissions?.includes('edit-contract-types') &&
                                      (contracttype.creator_id === auth.user?.id ||
                                          ...
                                          <Tooltip>
                                              <TooltipTrigger asChild>
                                                  <Button
                                                      variant="ghost"
                                                      size="sm"
                                                      onClick={() => openModal('edit', contracttype)}
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[{ label: t('Contract') }, { label: t('Contract Types') }]}
        ...
        >
            ...
            <Card className="shadow-sm">
                ...
                <CardContent className="bg-muted/50/50 border-b p-6">
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <PerPageSelector routeName="contract-types.index" filters={{ ...filters }} />
                ...
                <CardContent className="bg-muted/50/30 border-t px-4 py-2">
                    <Pagination
                        data={contracttypes || { current_page: 1, last_page: 1, per_page: 10, total: 0, from: 0, to: 0 }}
            ...
            <Dialog open={modalState.isOpen} onOpenChange={closeModal}>
                ...
                {modalState.mode === 'edit' && modalState.data && (
                    <EditContractType contracttype={modalState.data} onSuccess={closeModal} />
                )}
            </Dialog>

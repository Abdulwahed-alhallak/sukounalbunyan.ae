...
export default function EditWarning({ warning, onSuccess }: EditWarningProps) {
    ...
    const filteredWarningBies =
        allUsers?.filter(
            (user: any) => user.id.toString() !== data.employee_id || user.id.toString() === data.warning_by
        ) || [];

    useEffect(() => {
        if (
            data.employee_id &&
            data.warning_by === data.employee_id &&
            data.warning_by !== (warning.warning_by?.id || warning.warning_by)?.toString()
        ) {
            setData('warning_by', '');
        }
    }, [data.employee_id]);

    ...
    const tableColumns = [
        ...
        },
        {
            ...
            render: (value: string) => {
                ...
                return (
                    <span className={`px-2 py-1 rounded-full text-sm ${statusColors[normalizedStatus as keyof typeof statusColors] || 'bg-muted text-foreground'}`}>
                        {t(displayValue)}
                    </span>
                );
            }
        ...
        },
        {
            ...
            render: (_: any, employeetransfer: EmployeeTransfer) => (
                employeetransfer.document ? (
                    <a href={getImagePath(employeetransfer.document)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-foreground hover:text-foreground">
                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('manage-employee-transfers-status') && !['approved', 'rejected', 'cancelled'].includes(employeetransfer.status?.toLowerCase()) && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => setStatusModalItem(employeetransfer)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('view-employee-transfers') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => setViewingItem(employeetransfer)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('edit-employee-transfers') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', employeetransfer)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                            ...
                                            <div className="p-4 flex-1 min-h-0">
                                                <div className="grid grid-cols-2 gap-4 mb-4">
                                                    ...
                                                    <div className="text-xs min-w-0">
                                                        ...
                                                        <span className={`px-2 py-1 rounded-full text-xs font-medium inline-block ${
                                                            employeetransfer.status?.toLowerCase() === 'pending' ? 'bg-muted text-foreground' :
                                                            employeetransfer.status?.toLowerCase() === 'approved' ? 'bg-muted text-foreground' :
                                                            employeetransfer.status?.toLowerCase() === 'in progress' ? 'bg-muted text-foreground' :
                                                            employeetransfer.status?.toLowerCase() === 'rejected' ? 'bg-muted text-destructive' :
                                                            employeetransfer.status?.toLowerCase() === 'cancelled' ? 'bg-muted text-foreground' :
                                                ...
                                                <div className="grid grid-cols-2 gap-4 mb-4">
                                                    <div className="text-xs min-w-0">
                                                        ...
                                                        {employeetransfer.document ? (
                                                            <a href={getImagePath(employeetransfer.document)} target="_blank" className="text-foreground hover:text-foreground flex items-center gap-1 text-xs">
                                                ...
                                                <TooltipProvider>
                                                    {auth.user?.permissions?.includes('view-employee-transfers') && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => setViewingItem(employeetransfer)} className="h-9 w-9 p-0 text-foreground hover:text-foreground">
                                                                    <Eye className="h-4 w-4" />
                                                                </Button>
                                                            </TooltipTrigger>
                                                            <TooltipContent>
                                                    ...
                                                    {auth.user?.permissions?.includes('manage-employee-transfers-status') && !['approved', 'rejected', 'cancelled'].includes(employeetransfer.status?.toLowerCase()) && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => setStatusModalItem(employeetransfer)} className="h-9 w-9 p-0 text-foreground hover:text-foreground">
                                                                    <Play className="h-4 w-4" />
                                                                </Button>
                                                            </TooltipTrigger>
                                                            <TooltipContent>
                                                    ...
                                                    {auth.user?.permissions?.includes('edit-employee-transfers') && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => openModal('edit', employeetransfer)} className="h-9 w-9 p-0 text-foreground hover:text-foreground">
                                                                    <EditIcon className="h-4 w-4" />
                                                                </Button>
                                                            </TooltipTrigger>
                                                            <TooltipContent>

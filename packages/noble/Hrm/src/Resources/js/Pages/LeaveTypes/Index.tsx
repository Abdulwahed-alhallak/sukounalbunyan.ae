...
export default function Index() {
    ...
    const tableColumns = [
        ...
        ...(auth.user?.permissions?.some((p: string) =>
            ['view-leave-types', 'edit-leave-types', 'delete-leave-types'].includes(p)
        )
            ? [
                  {
                      key: 'actions',
                                      ...
                                      <Tooltip delayDuration={0}>
                                          <TooltipTrigger asChild>
                                              <Button
                                                  variant="ghost"
                                                  size="sm"
                                                  onClick={() => setViewingItem(leavetype)}
                                      ...
                                      <Tooltip delayDuration={0}>
                                          <TooltipTrigger asChild>
                                              <Button
                                                  variant="ghost"
                                                  size="sm"
                                                  onClick={() => openModal('edit', leavetype)}
                                                  className="h-8 w-8 p-0 text-foreground hover:text-foreground"
                                              >
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[{ label: t('Hrm'), url: route('hrm.index') }, { label: t('Leave Types') }]}
        ...
        >
            ...
            <Card className="shadow-sm">
                ...
                <CardContent className="bg-muted/50/50 border-b p-6">
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <PerPageSelector routeName="hrm.leave-types.index" filters={{ ...filters }} />
                            <div className="relative">
                                <FilterButton showFilters={showFilters} onToggle={() => setShowFilters(!showFilters)} />
                                {(() => {
                                    const activeFilters = [filters.is_paid].filter(
                ...
                {showFilters && (
                    <CardContent className="bg-muted/50/30 border-b p-6">
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-4">
                            <div>
                                <label className="mb-2 block text-sm font-medium text-foreground">{t('Is Paid')}</label>
                                <Select
                                    value={filters.is_paid}
                                    onValueChange={(value) => setFilters({ ...filters, is_paid: value })}
                                >
                                    <SelectTrigger>
                ...
                <CardContent className="bg-muted/50/30 border-t px-4 py-2">
                    <Pagination
                        data={leavetypes || { current_page: 1, last_page: 1, per_page: 10, total: 0, from: 0, to: 0 }}
            ...
            <Dialog open={modalState.isOpen} onOpenChange={closeModal}>
                ...
                {modalState.mode === 'edit' && modalState.data && (
                    <EditLeaveType leavetype={modalState.data} onSuccess={closeModal} />
                )}
            </Dialog>

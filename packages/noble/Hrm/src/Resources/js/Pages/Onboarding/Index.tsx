...
export default function Index() {
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[{ label: t('HRM'), url: route('hrm.index') }, { label: t('Onboarding') }]}
            pageTitle={t('Employee Onboarding')}
        >
                        ...
                        {items.length > 0 ? (
                            <div className="space-y-4">
                                ...
                                {onboardings.last_page > 1 && (
                                    <Pagination data={onboardings} routeName="hrm.onboarding.index" />
                                )}
                            </div>
                        ) : (
                            <NoRecordsFound
                                icon={UserPlus}
                                title={t('No onboarding records found')}
                                onCreateClick={() => {
                                    resetForm();
                                    setShowCreate(true);
                                }}
                                createButtonText={t('New Onboarding')}
                            />
                        )}
                    </CardContent>
                </Card>
            ...
            >
                <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>{editingItem ? t('Edit Onboarding') : t('New Onboarding')}</DialogTitle>
                    </DialogHeader>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="space-y-2 rounded-lg border bg-muted/30 p-3">
                                {form.checklist_items.map((item, idx) => (
                                    <div key={idx} className="flex items-center gap-3 rounded-lg p-2 hover:bg-muted/50">
                                        <Checkbox
                                        ...
                                        <label
                                            htmlFor={`check-${idx}`}
                                            className={`cursor-pointer text-sm ${item.completed ? 'text-muted-foreground line-through' : ''}`}
                                        >
                                            {t(item.label)}
                                        </label>
                                    </div>

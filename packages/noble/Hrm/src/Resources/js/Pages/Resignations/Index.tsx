...
import { Plus, Edit as EditIcon, Trash2, Eye, Tag as TagIcon, Download, FileImage, Check, X, ShieldAlert } from "lucide-react";
            ...
            render: (value: string, row: Resignation) => {
                ...
                return (
                    <div className="flex flex-col gap-1">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium w-fit ${statusColors[value as keyof typeof statusColors] || 'bg-muted'}`}>
                            {t(value?.charAt(0).toUpperCase() + value?.slice(1) || 'Unknown')}
                        </span>
                        {isMultiTier && row.manager_status && (
                            <span className={`px-2 py-1 rounded-full text-[10px] font-medium w-fit ${managerStatusColors[row.manager_status as keyof typeof managerStatusColors] || 'bg-muted'}`}>
        ...
        },
        {
            ...
            render: (_: any, resignation: Resignation) => (
                resignation.document ? (
                    <a href={getImagePath(resignation.document)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-foreground hover:text-foreground">
                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('manage-resignation-status') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openStatusModal(resignation)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('view-resignations') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => setViewingItem(resignation)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('edit-resignations') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', resignation)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                            ...
                                            <div className="p-4 flex-1 min-h-0">
                                                    ...
                                                    <div className="text-xs min-w-0">
                                                        ...
                                                        <div className="flex flex-col gap-1">
                                                            <span className={`px-2 py-1 rounded-full text-xs font-medium inline-block w-fit ${
                                                                resignation.status === 'pending' ? 'bg-warning/20 text-warning' :
                                                                resignation.status === 'accepted' ? 'bg-success/20 text-success' :
                                                                resignation.status === 'rejected' ? 'bg-destructive/20 text-destructive' :
                                                                'bg-muted'
                                                            }`}>
                                                                {t(resignation.status?.charAt(0).toUpperCase() + resignation.status?.slice(1) || 'Unknown')}
                                                            </span>
                                                            {(auth.user?.company_settings?.enable_multi_tier_approval === 'on' || true) && resignation.manager_status && (
                                                                <span className={`px-2 py-1 rounded-full text-[10px] font-medium w-fit ${
                                                                    resignation.manager_status === 'pending' ? 'bg-warning/20 text-warning' :
                                                                    resignation.manager_status === 'accepted' ? 'bg-success/20 text-success' :
                                                                    resignation.manager_status === 'rejected' ? 'bg-destructive/20 text-destructive' :
                                                                    'bg-muted'
                                                                }`}>
                                                                    Mgr {t(resignation.manager_status.charAt(0).toUpperCase() + resignation.manager_status.slice(1))}
                                                                </span>
                                                            )}
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="grid grid-cols-2 gap-4 mb-4">
                                                    <div className="text-xs min-w-0">
                                                        ...
                                                        {resignation.document ? (
                                                            <a href={getImagePath(resignation.document)} target="_blank" className="text-foreground hover:text-foreground flex items-center gap-1 text-xs">
                                                ...
                                                <TooltipProvider>
                                                    {auth.user?.permissions?.includes('view-resignations') && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => setViewingItem(resignation)} className="h-9 w-9 p-0 text-foreground hover:text-foreground">
                                                                    <Eye className="h-4 w-4" />
                                                                </Button>
                                                            </TooltipTrigger>
                                                            <TooltipContent>
                                                    ...
                                                    {auth.user?.permissions?.includes('manage-resignation-status') && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => openStatusModal(resignation)} className="h-9 w-9 p-0 text-foreground hover:text-foreground">
                                                                    <ShieldAlert className="h-4 w-4" />
                                                                </Button>
                                                            </TooltipTrigger>
                                                            <TooltipContent>
                                                    ...
                                                    {auth.user?.permissions?.includes('edit-resignations') && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" onClick={() => openModal('edit', resignation)} className="h-9 w-9 p-0 text-foreground hover:text-foreground">
                                                                    <EditIcon className="h-4 w-4" />
                                                                </Button>
                                                            </TooltipTrigger>
                                                            <TooltipContent>

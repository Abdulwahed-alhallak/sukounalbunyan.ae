...
import {
    InterviewFeedback,
    InterviewFeedbacksIndexProps,
    InterviewFeedbackFilters,
    InterviewFeedbackModalState,
} from './types';
...
export default function Index() {
    ...
    const handleFilter = () => {
        router.get(
            route('recruitment.interview-feedbacks.index'),
    ...
    const tableColumns = [
        {
            ...
            render: (value: any, row: any) => (
                <div>
                    ...
                    <div className="text-xs text-muted-foreground">{row.interview?.job_posting?.title || '-'}</div>
                </div>
            ),
        },
        {
            ...
            render: (value: number) => {
                ...
                return (
                    <div className="flex items-center gap-2">
                        <span className="font-small text-sm">{value}/5</span>
        ...
        },
        {
            ...
            render: (value: string) => {
                ...
                const getBadgeColor = (val: string) => {
                    switch (val) {
                        case '0':
                            return 'bg-muted text-foreground'; // Strong Hire
                        case '1':
                            return 'bg-muted text-foreground'; // Hire
                        case '2':
                            return 'bg-muted text-foreground'; // Maybe
                        case '3':
                        case '4':
                            return 'bg-muted text-destructive'; // Reject & Strong Reject
                        default:
                            return 'bg-muted text-foreground';
                    }
                };
                return (
                    <span className={`rounded-full px-2 py-1 text-sm ${getBadgeColor(value)}`}>{t(displayValue)}</span>
                );
            },
        },
        ...(auth.user?.permissions?.some((p: string) =>
            ['view-interview-feedbacks', 'edit-interview-feedbacks', 'delete-interview-feedbacks'].includes(p)
        )
            ? [
                  {
                      key: 'actions',
                                      ...
                                      <Tooltip delayDuration={0}>
                                          <TooltipTrigger asChild>
                                              <Button
                                                  variant="ghost"
                                                  size="sm"
                                                  onClick={() => setViewingItem(interviewfeedback)}
                                      ...
                                      <Tooltip delayDuration={0}>
                                          <TooltipTrigger asChild>
                                              <Button
                                                  variant="ghost"
                                                  size="sm"
                                                  onClick={() => openModal('edit', interviewfeedback)}
                                                  className="h-8 w-8 p-0 text-foreground hover:text-foreground"
                                              >
        ...
        >
            ...
            <Card className="shadow-sm">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="relative">
                                ...
                                {(() => {
                                    const activeFilters = [
                                        filters.interview_id !== 'all' ? filters.interview_id : '',
                                        filters.interviewer_id !== 'all' ? filters.interviewer_id : '',
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-4">
                            <div>
                                ...
                                <Select
                                    ...
                                    onValueChange={(value) => setFilters({ ...filters, interview_id: value })}
                                >
                                    <SelectTrigger>
                            ...
                            <div>
                                ...
                                <Select
                                    ...
                                    onValueChange={(value) => setFilters({ ...filters, recommendation: value })}
                                >
                                    <SelectTrigger>
                            ...
                            {interviewfeedbacks?.data?.length > 0 ? (
                                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                                    {interviewfeedbacks?.data?.map((interviewfeedback) => {
                                        ...
                                        const recommendationColors: any = {
                                            '0': 'bg-muted text-foreground',
                                            '1': 'bg-muted text-foreground',
                                            '2': 'bg-muted text-foreground',
                                            '3': 'bg-muted text-destructive',
                                            '4': 'bg-muted text-destructive',
                                        };
                                        const recommendationInfo = {
                                            label: options[interviewfeedback.recommendation] || 'No Recommendation',
                                            class:
                                                recommendationColors[interviewfeedback.recommendation] ||
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="bg-muted/50/50 flex items-center gap-3 border-b p-3">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <p className="text-xs text-muted-foreground">
                                                            {interviewfeedback.interview?.job_posting?.title ||
                                                                'No Job Title'}
                                                        </p>
                                                    </div>
                                                </div>
                                                <div className="flex-1 space-y-3 p-3">
                                                    <div className="min-w-0 text-xs">
                                                        ...
                                                        <p className="font-medium">
                                                            {interviewfeedback.interviewer_names || 'Unknown'}
                                                        </p>
                                                    </div>
                                                            ...
                                                            {interviewfeedback.overall_rating ? (
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <span className="text-sm font-medium">
                                                                        {interviewfeedback.overall_rating}/5
                                                                    </span>
                                                                </div>
                                                            ) : (
                                                        ...
                                                        <div className="min-w-0 text-xs">
                                                            ...
                                                            <p className="font-medium">
                                                                {interviewfeedback.created_at
                                                                    ? formatDate(interviewfeedback.created_at)
                                                                    : '-'}
                                                            </p>
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="bg-muted/50/50 mt-auto flex flex-shrink-0 items-center justify-between border-t p-3">
                                                    <span
                                                        className={`rounded-full px-2 py-1 text-xs ${recommendationInfo.class}`}
                                                    >
                                                                ...
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                            ...
                                                                            size="sm"
                                                                            onClick={() =>
                                                                                setViewingItem(interviewfeedback)
                                                                            }
                                                                ...
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                            ...
                                                                            size="sm"
                                                                            onClick={() =>
                                                                                openModal('edit', interviewfeedback)
                                                                            }
                                                                            className="h-9 w-9 p-0 text-foreground hover:bg-muted/50 hover:text-foreground"
                                                                        >
                                                                            <EditIcon className="h-4 w-4" />
                                                                ...
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                            ...
                                                                            size="sm"
                                                                            onClick={() =>
                                                                                openDeleteDialog(interviewfeedback.id)
                                                                            }
                ...
                <CardContent className="bg-muted/50/30 border-t px-4 py-2">
                    <Pagination
                        data={interviewfeedbacks || { current_page: 1, last_page: 1, per_page: 10, total: 0, from: 0, to: 0 }}
                        routeName="recruitment.interview-feedbacks.index"
            ...
            <Dialog open={modalState.isOpen} onOpenChange={closeModal}>
                ...
                {modalState.mode === 'edit' && modalState.data && (
                    <Edit interviewfeedback={modalState.data} onSuccess={closeModal} />
                )}
            </Dialog>

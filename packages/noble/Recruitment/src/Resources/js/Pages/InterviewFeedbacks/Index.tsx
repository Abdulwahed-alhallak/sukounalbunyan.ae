        ...
        },
        {
            ...
            render: (value: string) => {
                ...
                const getBadgeColor = (val: string) => {
                    switch (val) {
                        case '0': return 'bg-muted text-foreground'; // Strong Hire
                        case '1': return 'bg-muted text-foreground';   // Hire
                        case '2': return 'bg-muted text-foreground'; // Maybe
                        case '3': case '4': return 'bg-muted text-destructive'; // Reject & Strong Reject
                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('view-interview-feedbacks') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => setViewingItem(interviewfeedback)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('edit-interview-feedbacks') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', interviewfeedback)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                ...
                {showFilters && (
                    <CardContent className="p-6 bg-muted/50/30 border-b">
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Interview')}</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Interviewer')}</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Recommendation')}</label>
                            ...
                            {interviewfeedbacks?.data?.length > 0 ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                    {interviewfeedbacks?.data?.map((interviewfeedback) => {
                                        const options: any = { "0": "Strong Hire", "1": "Hire", "2": "Maybe", "3": "Reject", "4": "Strong Reject" };
                                        const recommendationColors: any = { "0": "bg-muted text-foreground", "1": "bg-muted text-foreground", "2": "bg-muted text-foreground", "3": "bg-muted text-destructive", "4": "bg-muted text-destructive" };
                                        const recommendationInfo = { label: options[interviewfeedback.recommendation] || 'No Recommendation', class: recommendationColors[interviewfeedback.recommendation] || 'bg-muted text-foreground' };
                                                    ...
                                                    <div className="grid grid-cols-2 gap-4">
                                                        <div className="text-xs min-w-0">
                                                            ...
                                                            {interviewfeedback.overall_rating ? (
                                                                ...
                                                                <span className="text-sm text-muted-foreground">{t('No rating')}</span>
                                                            )}
                                                        </div>
                                                        <div className="text-xs min-w-0">
                                                        ...
                                                        <TooltipProvider>
                                                            {auth.user?.permissions?.includes('view-interview-feedbacks') && (
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => setViewingItem(interviewfeedback)} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                            <Eye className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>
                                                            ...
                                                            {auth.user?.permissions?.includes('edit-interview-feedbacks') && (
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => openModal('edit', interviewfeedback)} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                            <EditIcon className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>

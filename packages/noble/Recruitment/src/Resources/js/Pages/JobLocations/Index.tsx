    ...
    const tableColumns = [
        ...
        },
        {
            ...
            render: (value: any, row: any) => (
                <span className={`text-sm ${row.remote_work ? 'text-foreground font-medium' : ''}`}>
        ...
        },
        {
            ...
            sortable: false,
            render: (value: any, row: any) => (
                <span className={`px-2 py-1 rounded-full text-sm ${row.remote_work ? 'bg-muted text-foreground' : 'bg-muted text-foreground'}`}>
                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('view-job-locations') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => setViewingItem(joblocation)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('edit-job-locations') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', joblocation)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                                    ...
                                    {joblocations?.data?.map((joblocation) => (
                                        <Card key={joblocation.id} className="p-0 hover:shadow-lg transition-all duration-200 relative overflow-hidden flex flex-col h-full min-w-0">
                                            <div className="p-4 bg-gradient-to-r from-primary/5 to-transparent border-b flex-shrink-0">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <h3 className="font-semibold text-sm text-foreground">{joblocation.name}</h3>
                                            ...
                                            <div className="p-4 flex-1 min-h-0">
                                                <div className="grid grid-cols-1 gap-4 mb-4">
                                                    <div className="text-xs min-w-0">
                                                        ...
                                                        <p className={`font-medium ${joblocation.remote_work ? 'text-foreground' : 'text-foreground'}`}>
                                            ...
                                            <div className="flex justify-between items-center p-3 border-t bg-muted/50/50 flex-shrink-0 mt-auto">
                                                <span className={`px-2 py-1 rounded-full text-xs ${joblocation.status ? 'bg-muted text-foreground' : 'bg-muted text-destructive'}`}>
                                                    ...
                                                    <TooltipProvider>
                                                        {auth.user?.permissions?.includes('view-job-locations') && (
                                                            <Tooltip delayDuration={300}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => setViewingItem(joblocation)} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                        <Eye className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                                <TooltipContent>
                                                        ...
                                                        {auth.user?.permissions?.includes('edit-job-locations') && (
                                                            <Tooltip delayDuration={300}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', joblocation)} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                        <EditIcon className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                                <TooltipContent>

                ...
                <div className="flex items-center gap-1 min-w-fit">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('send-offer-emails') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => handleSendEmail(offer.id)} className="h-8 w-8 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                        ...
                        {offer.status === '2' && auth.user?.permissions?.includes('download-offer-letters') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => window.open(offer.download_url, '_blank')} className="h-8 w-8 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                        ...
                        {offer.status === '2' && !Boolean(offer.converted_to_employee) && auth.user?.permissions?.includes('convert-offers-to-employees') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('recruitment.offers.convert-to-employee', offer.id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                        ...
                        {Boolean(offer.converted_to_employee) && offer.employee_id && auth.user?.permissions?.includes('view-offer-employees') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('hrm.employees.show', offer.employee_id))} className="h-8 w-8 p-0 text-foreground hover:text-foreground hover:bg-accent">
                        ...
                        {auth.user?.permissions?.includes('view-offers') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => setViewingItem(offer)} className="h-8 w-8 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                        ...
                        {auth.user?.permissions?.includes('edit-offers') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', offer)} className="h-8 w-8 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                ...
                {showFilters && (
                    <CardContent className="p-6 bg-muted/50/30 border-b">
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-7 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Candidate')}</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Approval Status')}</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Offer Date')}</label>
                                <DatePicker
                            ...
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">{t('Expiration Date')}</label>
                                <DatePicker
                            ...
                            {offers?.data?.length > 0 ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                    {offers?.data?.map((offer) => {
                                        ...
                                        const statusColors: any = { "0": "bg-muted text-foreground", "1": "bg-muted text-foreground", "2": "bg-muted text-foreground", "3": "bg-muted text-foreground", "4": "bg-muted text-destructive", "5": "bg-muted text-foreground" };
                                        const statusInfo = { label: statusOptions[offer.status] || 'Draft', class: statusColors[offer.status as keyof typeof statusColors] || 'bg-muted text-foreground' };
                                                ...
                                                <div className="flex-1 p-3 space-y-3">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-4">
                                                        ...
                                                        <div className="text-xs min-w-0">
                                                            ...
                                                            <p className={`font-medium ${isExpired ? 'text-destructive' : ''}`}>{offer.expiration_date ? formatDate(offer.expiration_date) : '-'}</p>
                                                        </div>
                                                    </div>
                                                    <div className="grid grid-cols-2 gap-4">
                                                        <div className="text-xs min-w-0">
                                                        ...
                                                        <TooltipProvider>
                                                            {auth.user?.permissions?.includes('send-offer-emails') && (
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => handleSendEmail(offer.id)} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                            <Mail className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>
                                                            ...
                                                            {offer.status === '2' && auth.user?.permissions?.includes('download-offer-letters') && (
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => window.open(offer.download_url, '_blank')} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                            <Download className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>
                                                            ...
                                                            {offer.status === '2' && !Boolean(offer.converted_to_employee) && auth.user?.permissions?.includes('convert-offers-to-employees') && (
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => router.get(route('recruitment.offers.convert-to-employee', offer.id))} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                            <UserPlus className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>
                                                            ...
                                                            {Boolean(offer.converted_to_employee) && offer.employee_id && auth.user?.permissions?.includes('view-offer-employees') && (
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => router.get(route('hrm.employees.show', offer.employee_id))} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-accent">
                                                                            <User className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>
                                                            ...
                                                            {auth.user?.permissions?.includes('view-offers') && (
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => setViewingItem(offer)} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                            <Eye className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>
                                                            ...
                                                            {auth.user?.permissions?.includes('edit-offers') && (
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => openModal('edit', offer)} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                            <EditIcon className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>

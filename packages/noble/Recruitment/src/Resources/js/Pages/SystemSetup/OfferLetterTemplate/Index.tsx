...
export default function OfferLetterTemplate() {
    ...
    const languages = settings?.languages
        ? Object.entries(settings.languages)?.map(([code, data]: [string, any]) => ({
              code,
              name: data.name || data,
    ...
    const templateAI = useFormFields(
        'aiField',
        formSettings,
        (field, value) => {
    ...
    const saveSettings = () => {
        ...
        router.post(
            route('recruitment.offer-letter-template.update'),
            {
            ...
            },
            {
                ...
                onError: (errors) => {
                    setIsLoading(false);
                    const errorMessage =
                        errors.error || Object.values(errors).join(', ') || t('Failed to save template');
                    toast.error(errorMessage);
                },
            }
        );
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
                                        {placeholders?.map((placeholder, index) => (
                                            <div key={index} className="rounded border bg-muted/50 p-2">
                                                <div className="text-sm font-medium text-foreground">
                                                    {placeholder.name}
                                                </div>
                                                <div className="text-xs text-foreground">{placeholder.value}</div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                    ...
                                    <div className="mb-2 flex items-center justify-between">
                                        ...
                                        {canEdit && (
                                            <div className="flex gap-2">
                                                {templateAI?.map((field) => (
                                                    <div key={field.id}>{field.component}</div>
                                                ))}
                                            </div>
                                        )}
                                    </div>
                                    <div className={errors.template ? 'rounded-md border border-destructive' : ''}>
                                        <RichTextEditor

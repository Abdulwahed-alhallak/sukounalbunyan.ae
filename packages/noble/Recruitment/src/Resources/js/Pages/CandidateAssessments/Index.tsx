        ...
        },
        {
            ...
            render: (value: string) => {
                ...
                const getStatusColor = (status: string) => {
                    switch(status) {
                        case '0': return 'bg-muted text-foreground'; // Pass - Green
                        case '1': return 'bg-muted text-destructive';     // Fail - Red
                        case '2': return 'bg-muted text-foreground'; // Pending - Yellow
                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('view-candidate-assessments') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => setViewingItem(candidateassessment)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                        ...
                        {auth.user?.permissions?.includes('edit-candidate-assessments') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', candidateassessment)} className="h-8 w-8 p-0 text-foreground hover:text-foreground">
                            ...
                            {candidateassessments?.data?.length > 0 ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                    {candidateassessments?.data?.map((candidateassessment) => {
                                        ...
                                        const statusColors: any = {"0":"bg-muted text-foreground","1":"bg-muted text-destructive","2":"bg-muted text-foreground"};
                                        const statusInfo = { label: statusOptions[candidateassessment.pass_fail_status] || 'Pending', class: statusColors[candidateassessment.pass_fail_status as keyof typeof statusColors] || 'bg-muted text-foreground' };
                                            ...
                                            <Card key={candidateassessment.id} className="flex flex-col h-full hover:shadow-md transition-shadow duration-200">
                                                <div className="flex items-center gap-3 p-3 border-b bg-muted/50/50">
                                                    <div className="p-2 bg-foreground/10 rounded-lg flex-shrink-0">
                                                        <ClipboardCheckIcon className="h-5 w-5 text-foreground" />
                                                    </div>
                                                ...
                                                <div className="flex-1 p-3 space-y-3">
                                                    ...
                                                    <div className="text-xs min-w-0">
                                                        ...
                                                        <div className="w-full bg-muted rounded-full h-2 mb-1">
                                                            <div className="bg-foreground h-2 rounded-full transition-all duration-300" style={{width: `${percentage}%`}}></div>
                                                        </div>
                                                        ...
                                                        <TooltipProvider>
                                                            {auth.user?.permissions?.includes('view-candidate-assessments') && (
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => setViewingItem(candidateassessment)} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                            <Eye className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>
                                                            ...
                                                            {auth.user?.permissions?.includes('edit-candidate-assessments') && (
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => openModal('edit', candidateassessment)} className="h-9 w-9 p-0 text-foreground hover:text-foreground hover:bg-muted/50">
                                                                            <EditIcon className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>

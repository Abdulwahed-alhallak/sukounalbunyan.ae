...
import {
    CandidateAssessment,
    CandidateAssessmentsIndexProps,
    CandidateAssessmentFilters,
    CandidateAssessmentModalState,
} from './types';
...
export default function Index() {
    ...
    const handleFilter = () => {
        router.get(
            route('recruitment.candidate-assessments.index'),
    ...
    const tableColumns = [
        ...
        },
        {
            ...
            render: (value: string) => {
                ...
                const getStatusColor = (status: string) => {
                    switch (status) {
                        case '0':
                            return 'bg-muted text-foreground'; // Pass - Green
                        case '1':
                            return 'bg-muted text-destructive'; // Fail - Red
                        case '2':
                            return 'bg-muted text-foreground'; // Pending - Yellow
                        default:
                ...
                return (
                    <span className={`rounded-full px-2 py-1 text-sm ${getStatusColor(value)}`}>{t(displayValue)}</span>
                );
            },
        },
        {
        ...
        ...(auth.user?.permissions?.some((p: string) =>
            ['view-candidate-assessments', 'edit-candidate-assessments', 'delete-candidate-assessments'].includes(p)
        )
            ? [
                  {
                      key: 'actions',
                                      ...
                                      <Tooltip delayDuration={0}>
                                          <TooltipTrigger asChild>
                                              <Button
                                                  variant="ghost"
                                                  size="sm"
                                                  onClick={() => setViewingItem(candidateassessment)}
                                      ...
                                      <Tooltip delayDuration={0}>
                                          <TooltipTrigger asChild>
                                              <Button
                                                  variant="ghost"
                                                  size="sm"
                                                  onClick={() => openModal('edit', candidateassessment)}
                                                  className="h-8 w-8 p-0 text-foreground hover:text-foreground"
                                              >
        ...
        >
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-4">
                            ...
                            <div>
                                ...
                                <Select
                                    ...
                                    onValueChange={(value) => setFilters({ ...filters, pass_fail_status: value })}
                                >
                                    <SelectTrigger>
                                ...
                                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                                    {candidateassessments?.data?.map((candidateassessment) => {
                                        ...
                                        const statusColors: any = {
                                            ...
                                            '1': 'bg-muted text-destructive',
                                            '2': 'bg-muted text-foreground',
                                        };
                                        const statusInfo = {
                                            label: statusOptions[candidateassessment.pass_fail_status] || 'Pending',
                                            class:
                                                statusColors[
                                                    candidateassessment.pass_fail_status as keyof typeof statusColors
                                                ] || 'bg-muted text-foreground',
                                        };

                                        return (
                                            <Card
                                                key={candidateassessment.id}
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 space-y-3 p-3">
                                                    ...
                                                    <div className="min-w-0 text-xs">
                                                        ...
                                                        <p className="font-medium">
                                                            {candidateassessment.assessment_date
                                                                ? formatDate(candidateassessment.assessment_date)
                                                                : '-'}
                                                        </p>
                                                    </div>
                                                    <div className="min-w-0 text-xs">
                                                                ...
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                            ...
                                                                            size="sm"
                                                                            onClick={() =>
                                                                                setViewingItem(candidateassessment)
                                                                            }
                                                                ...
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                            ...
                                                                            size="sm"
                                                                            onClick={() =>
                                                                                openModal('edit', candidateassessment)
                                                                            }
                                                                            className="h-9 w-9 p-0 text-foreground hover:bg-muted/50 hover:text-foreground"
                                                                        >
                                                                            <EditIcon className="h-4 w-4" />
                                                                ...
                                                                <Tooltip delayDuration={300}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                            ...
                                                                            size="sm"
                                                                            onClick={() =>
                                                                                openDeleteDialog(candidateassessment.id)
                                                                            }
            ...
            <Dialog open={modalState.isOpen} onOpenChange={closeModal}>
                ...
                {modalState.mode === 'edit' && modalState.data && (
                    <Edit candidateassessment={modalState.data} onSuccess={closeModal} />
                )}
            </Dialog>

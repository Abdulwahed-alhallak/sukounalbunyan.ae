...
echo "--- DEEP AUDIT & SUBSCRIPTION SYNCHRONIZATION STARTED ---\n";

// Phase 1: Sync allNobleArchitecture Modules
echo "1. Scanning packages/dionone...\n";
$modulesPath = base_path('packages/dionone');
...
foreach ($directories as $dir) {
    ...
    if (File::exists($jsonPath)) {
        ...
        // Optionally read pricing if we want, but basically we force enable:
        $addon->save();
        echo "   [ENABLED] $name\n";
    }
}
...
// Phase 2: Professional Plan Enforcement (Noble & Dion)
echo "3. Securing Noble & Dion Custom Subscriptions...\n";
...
upgradeCompanyToPro('admin@dion.sy', $proPlan, $allModules); // If Dion Creative exists as admin@dion.sy
...
if (!$standardPlan && $proPlan) {
    $standardPlan = Plan::create([
        ...
        'modules' => ['Taskly', 'Hrm'], // Tasks AND HRM, but NOT CRM
        'status' => 1,
        'number_of_users' => 20,
        'created_by' => 1,
    ]);

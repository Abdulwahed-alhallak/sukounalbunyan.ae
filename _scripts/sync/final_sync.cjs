...
const filesToSync = [
    { local: '_scripts/activate_modules.php', remote: 'activate_modules.php' }
];

// Read all files in public/build to sync
...
function runRemoteCommands() {
    const commands = [
        `cd ${config.remotePath} && /opt/alt/php82/usr/bin/php activate_modules.php`,
        `cd ${config.remotePath} && sed -i "s|^APP_URL=.*|APP_URL=https://sukounalbunyan.ae/backend|g" .env`,
        `cd ${config.remotePath} && /opt/alt/php82/usr/bin/php artisan cache:clear`,
        `cd ${config.remotePath} && /opt/alt/php82/usr/bin/php artisan config:clear`,
        `cd ${config.remotePath} && rm activate_modules.php`
    ];

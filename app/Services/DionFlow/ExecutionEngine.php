...
namespace App\Services\DionFlow;
    ...
    public static function run(Workflow $workflow, $payload): void
    {
        ...
        foreach ($steps as $step) {
            try {
                ...
                Log::error("DionFlow Execution Error: Workflow [{$workflow->name}], Step ID [{$step->id}]: " . $e->getMessage());
                // Depending on strictness, we might throw or continue.
                // In DionFlow, we log and continue so partial execution works.
            }
        }
    }
    ...
    private static function processAction(WorkflowStep $step, $payload): void
    {
        ...
        switch ($step->action_type) {
            case 'send_email':
                ...
                Log::info("DionFlow: Mock Sending Email to " . ($config['to'] ?? 'Unknown'));
                break;

            case 'create_task':
                // Check if Taskly module exists
                if (class_exists(\DionONE\Taskly\Entities\Task::class)) {
                    // Logic to create a project task based on payload (e.g. Lead -> New Task)
                    Log::info("DionFlow: Task Creation Hooked!");
                } else {
                    Log::warning("DionFlow: create_task failed. Taskly module missing.");
                }
                break;
            
            case 'generate_pdf_invoice':
                // Integrate with our existing backend PDF generator!
                Log::info("DionFlow: Autogenerating PDF...");
                break;

            default:
                Log::warning("DionFlow: Unknown Action Type - {$step->action_type}");
                break;
        }
    }
}

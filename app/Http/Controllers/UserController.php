...
class UserController extends Controller
{
    public function index()
    {
        if(Auth::user()->can('manage-users')){
            ...
            $plans = \App\Models\Plan::where('status', true)->get(['id', 'name']);
    ...
    public function store(StoreUserRequest $request)
    {
        if(Auth::user()->can('create-users')){
            ...
            // Notification Center Trigger - Welcome New User
            ...
            // Notification Center Trigger - Notify Creator (Admin/Company)
            if (Auth::check() && Auth::id() !== $user->id) {
                \App\Models\UserNotification::send(
                    Auth::id(),
                    'system',
                    'New User Created',
                    "User {$user->name} ({$user->email}) was successfully added to the system.",
                    [
                        'icon' => 'UserPlus',
    ...
    public function assignPlan(\Illuminate\Http\Request $request, User $user)
    {
        if (Auth::user()->type === 'superadmin' && $user->type === 'company') {
            ...
            // Assign Plan globally using helper with proper modules
            assignPlan($request->plan_id, 'Lifetime', $modulesStr, null, $user->id);

            return back()->with('success', __('User plan successfully assigned.'));
        }
        else {
            return back()->with('error', __('Permission denied.'));
        }

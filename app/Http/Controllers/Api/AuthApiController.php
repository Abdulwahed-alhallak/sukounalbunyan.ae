...
class AuthApiController extends Controller
{
        ...
        try {
            ...
            if (!$user || !Hash::check($request->password, $user->password)) {
                ...
                SecurityAuditLog::record(
                    ...
                    SecurityAuditLog::recentFailedAttempts($request->ip()) >= 5
                );
            ...
            SecurityAuditLog::record('login', $user->id, null,
                SecurityAuditLog::isNewIpForUser($user->id, $request->ip())
            );
    ...
    public function logout(Request $request)
    {
        $user = $request->user();
        if (!empty($user)) {
            SecurityAuditLog::record('logout', $user->id);
    ...
    public function changePassword(Request $request)
    {
        try {
            ...
            SecurityAuditLog::record('password_changed', $user->id);
        ...
        try {
            ...
            if ($user) {
                ...
                if ($request->hasFile('profile')) {
                    ...
                    if (!empty($user->avatar) && strpos($user->avatar, 'avatar.png') === false && getImageUrlPrefix()) {
    ...
    public function deleteAccount(Request $request)
    {
        try {
            $user = $request->user();
            SecurityAuditLog::record('account_deleted', $user->id, 'Self-deletion via API');
            $user->delete();

...
class RegisteredUserController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            // Notification Center Trigger - Notify Super Admins
            \App\Models\UserNotification::sendToSuperAdmins(
                'system',
                'New Company Registered',
                "Company {$user->name} ({$user->email}) has just registered on the platform.",
                [
                    'icon' => 'Building',
                    'action_url' => route('dashboard'), // Superadmin's immediate path
                ]
            );

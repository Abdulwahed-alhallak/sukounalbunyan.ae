...
conn.on('ready', () => {
    conn.exec('cd domains/sukounalbunyan.ae/public_html/backend && /opt/alt/php82/usr/bin/php artisan tinker --execute="DB::select(\"SHOW COLUMNS FROM projects WHERE Field = \'status\'\");"', (err, stream) => {
        if (err) throw err;
